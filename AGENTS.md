...
**Session continuity:** Use a local handoff when available, then verify current Git state and user direction. Handoffs and session recovery notes are local working material: never stage, commit, push, or include them in a PR. A request to create or update a handoff does not authorize committing it.
...
- **Keep recovery notes local.** Store handoffs outside the repository. Put concise change and validation summaries in PR descriptions; do not copy session recovery notes, raw transcripts, credentials, private tool payloads, or hidden reasoning into repository documentation.
