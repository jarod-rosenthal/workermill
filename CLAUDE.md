...
***REMOVED******REMOVED******REMOVED*** DO NOT Edit worker/epic/*.ts Files
...
***REMOVED*** 2. Kill zombie processes on all ports (CRITICAL — skip this and Vite may start on 5174)
lsof -ti :5173 -ti :5174 -ti :3001 2>/dev/null | xargs -r kill -9

***REMOVED*** 3. Start services (--skip-db if PostgreSQL is already running)
./bin/local-workermill start --skip-db

***REMOVED*** 4. Verify frontend is on port 5173 (NOT 5174)
cat .local-workermill/frontend.log
***REMOVED*** Must show: Local: http://localhost:5173/

***REMOVED*** 5. Tell user to hard-refresh browser: Ctrl+Shift+R
```
...
***REMOVED******REMOVED******REMOVED*** worker/epic/ Has Diverged .ts and .js Files (CRITICAL)
