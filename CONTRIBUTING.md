***REMOVED*** Contributing to WorkerMill

Thank you for your interest in contributing to WorkerMill.

***REMOVED******REMOVED*** How to Contribute

***REMOVED******REMOVED******REMOVED*** Reporting Bugs
Open an issue with:
- Steps to reproduce
- Expected vs actual behavior
- Environment details (OS, Node version, browser)

***REMOVED******REMOVED******REMOVED*** Feature Requests
Open an issue describing the feature and its use case.

***REMOVED******REMOVED******REMOVED*** Pull Requests
1. Fork the repository
2. Create a feature branch (`git checkout -b feature/my-feature`)
3. Make your changes
4. Run type checking (`cd api && npm run typecheck && cd ../frontend && npx tsc -b`)
5. Run tests (`cd api && npm run test`)
6. Commit with a clear message
7. Open a PR against `main`

***REMOVED******REMOVED******REMOVED*** Development Setup
See the [Quick Start](***REMOVED***quick-start) section in README.md.

***REMOVED******REMOVED*** Code Style
- TypeScript throughout (API, frontend, agent, worker)
- Prettier for formatting (runs automatically via hooks)
- ESLint for linting (`npm run lint` in api/ and frontend/)

***REMOVED******REMOVED*** Architecture Overview
See [docs/agent/architecture.md](docs/agent/architecture.md) for the full architecture guide.

***REMOVED******REMOVED*** Code of Conduct
See [CODE_OF_CONDUCT.md](CODE_OF_CONDUCT.md).
