...
Checkpoint: 2026-09-06 22:23 UTC. Integration branch: `reliability/core`. Latest qualified implementation: `1c33f857` — **1,601 tests passed, zero failed, one existing skip, 110 files**. Typecheck passed; build passed at `0b7a708c` (same production source). Qualification uses the exact locked dependency graph, Node 22.22.2/Linux; full suite took 31.88 seconds. Core is clean before this documentation edit. Accepted: R15/R23, all R16/R17, R18a, all R18b, R18c1 and R18c2.

R18c2 candidate `bb51a147` integrated as `6f99264b`. Root correction `0b7a708c` retains review totals before cancellation/empty-output checks and tests synchronous failures, distinct retries, and unresolved usage promises. First full run failed two lifecycle tests because their CostTracker doubles lacked recordCall. Root replaced them with the real tracker (`1c33f857`); the same cleanup assertions and full suite now pass. No downstream dispatch occurred while the checks were red.

Next dispatch: R18c3a adds validated, allowlisted manifest/run-output ledger snapshots and whole-run sum tests. It owns run-manifest.ts, orchestrator.ts/types, narrow ledger output calls and associated tests. R18c3b then applies unique calls to cumulative session/UI totals and displays unknown/incomplete estimates. R22 reference/migration docs and R24 final qualification follow. Optional R19 budgets and R21 comparison work remain outside the first release scope. See the plan/queue for exact splits.

Qualification worktree: `/home/user/github/workermill-qualification-cont`, branch `reliability/qualification-cont`, fast-forwarded from core before checks. Its node_modules links to `/home/user/github/workermill-r17-cont/node_modules`, independently installed with npm ci and cache `/tmp/workermill-qualification-npm-cache`. Original core dependencies were preserved; they differ from the lockfile. Run future release qualification in the locked worktree.

R17 package/PTY tests passed four cases at `a9b0976e`; rerun at final production HEAD. Commands: `npm run build`, then `npm_config_cache=/tmp/workermill-qualification-npm-cache npm run test:package-os`. Linux/macOS × Node 22.12.0/22.22.2 CI is configured but remote jobs were not executed. OS sandbox tests explicitly skip unavailable prerequisites; supported prerequisites are provisioned in CI.
