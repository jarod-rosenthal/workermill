...
Checkpoint: 2026-09-06 22:34 UTC. Integration branch: `reliability/core`. Latest qualified implementation: `0efe4934` — **1,605 passed, zero failed, one existing skip, 110 files**; typecheck passed at 4c7cbc05 (same production), build passed at 0efe4934. Full suite 31.71 seconds, Node 22.22.2/Linux, exact package-lock dependencies.

Accepted: R15/R23, all R16/R17, R18a/b, R18c1/c2/c3a. R18c3a manifests validate/allowlist per-call snapshots and matching top-level totals; result, final output, and saved manifest agree on planner/worker/failed-review/review sums. Root adopted shared zero-placeholder handling for children. First full run had one fixture expectation still using the old review tokens; corrected from its actual fixture observation, then all checks passed.

Next bounded task: R18c3b shared cumulative session accounting and unknown/incomplete UI summaries. It owns session.ts, run-command accounting, useAgent/useOrchestrator/Root wiring, session/run/stats cost readers, cost-tracker role/format helpers, and focused tests. Preserve historical totals without inventing historical calls; repeated cumulative snapshots apply only new call IDs. R22 reference/migration docs and R24 final qualification follow. Optional R19 budgets and R21 comparison harness remain outside this first release.

Qualification worktree `/home/user/github/workermill-qualification-cont`, branch `reliability/qualification-cont`, is fast-forwarded from core before checks. node_modules links to `/home/user/github/workermill-r17-cont/node_modules`, independently installed via npm ci with cache `/tmp/workermill-qualification-npm-cache`. Original core dependencies were preserved and differ from the lockfile. R17 four package/PTY cases passed at a9b0976e; rerun final HEAD after build using `npm_config_cache=/tmp/workermill-qualification-npm-cache npm run test:package-os`. Linux/macOS × Node 22.12.0/22.22.2 CI configured; remote jobs unexecuted.
