...
**Current state: first-release reliability work is locally complete.** Checkpoint: 2026-09-06 22:53 UTC. Repository `/home/user/github/workermill`, branch `reliability/core`. Qualified candidate **67e19c20**: **1,614 tests passed, zero failed, one existing skip, 111 files; four installed-package/PTY tests passed; typecheck and build passed**. Exact commands, limitations and reviewed contracts: [qualification report](docs/recovery/r24-qualification.md). The initial evidence-only commit follows that candidate; subsequent CI and test-fixture corrections are recorded below. Production code is unchanged.

Remote qualification update (2026-09-06 23:18 UTC): the user explicitly authorized pushing reliability/core. Candidate 7e08a94a was pushed successfully and CI was manually dispatched against that exact SHA: https://github.com/jarod-rosenthal/workermill/actions/runs/34066504739 . That first matrix completed with failures documented below. No merge or release is authorized. The earlier no-push statements below describe the prior checkpoint. Next action: inspect all four job results, correct bounded CI failures if needed, and persist final remote evidence.

CI run 34066504739 failed at unit tests in all four jobs after successful install/typecheck/build; package checks did not run. Node22.22.2 Linux: 4 failed/1604 passed/7 skipped; macOS: 8 failed/1605 passed/2 skipped. Raw logs: /tmp/workermill-ci-linux.log and /tmp/workermill-ci-macos.log (portable summary here). Bounded correction: canonicalize test temp roots, use relative marker paths/PATH Node in chat cancellation, keep OS-unavailable sentinel inside workspace to reach the intended check, load Ubuntu's packaged bwrap AppArmor profile and probe actual user-namespace startup. Do not weaken production path checks or skip supported CI containment tests. Correction committed as `55629f3c`: local full suite exit 0 (1,614 passed, one existing skip, 111 files, 31.47 seconds); typecheck exit 0. Pushed successfully. Second CI run https://github.com/jarod-rosenthal/workermill/actions/runs/34067913059 targets that exact commit; remote results pending. Coordinator owns monitoring and bounded corrections; no active implementation workers.

Second CI run 34067913059: both macOS unit suites passed (1,613 passed, two skips), then package tests had three passes and one native PTY startup failure. Linux stopped at the explicit probe: `bwrap: setting up uid map: Permission denied`. Inspection of Ubuntu apparmor-profiles 4.0.1really4.0.1-0ubuntu0.24.04.7 confirms the required packaged policy lives at `/usr/share/apparmor/extra-profiles/bwrap-userns-restrict`, not `/etc/apparmor.d/bwrap`; next correction installs that package and loads that exact profile, preserving child capability denial and global restrictions. node-pty 1.1.0 (latest stable) ships macOS spawn-helper mode 0644, confirmed in locked dependencies and upstream issue microsoft/node-pty#850. It is only a test terminal driver, with no production caller; next correction adds owner execute permission only to its temporary installed test helper. Scope: workflow, package test harness, handoff. No production or dependency-version changes. Requalify types/package test locally, push, rerun full CI matrix.

Current remote candidate: `e0c5e6355feb8c539cf0cf752747596c9493b657`, pushed. Local typecheck exit 0; installed-package tests four passed, 10.96 seconds. Full matrix run https://github.com/jarod-rosenthal/workermill/actions/runs/34069185187 pending. Next action: inspect all four results and retain exact failure evidence if any.

Third CI run 34069185187 failed: Linux Node22.22.2 had 8 failed/1606 passed/one skip. The bwrap probe passes, but sandbox-runtime 0.0.46 apply-seccomp creates a second namespace; Ubuntu's packaged profile denies its child capabilities (`write /proc/self/setgroups ... Permission denied`). No global restriction or child capability denial will be disabled to get green CI. Linux hosted qualification remains blocked pending a compatible runtime/host policy. macOS units pass; PTY now starts but Ink detects inherited CI=true and disables interactive rendering. Next bounded correction: set CI=false and CONTINUOUS_INTEGRATION=false only for the real interactive PTY child, preserving all input/cancel/status assertions. Coordinator limits supported Linux qualification to Ubuntu22.04; plan R17 requires Linux/macOS and both Node versions, not a particular Ubuntu release. Matrix is explicitly pinned to Ubuntu22.04 rather than changing Ubuntu24.04 protections. Ubuntu24.04 remains an open compatibility finding; do not claim it passed. Local package test/typecheck then push and rerun the supported matrix.
...
3. Monitor CI run 34069185187 for Linux/macOS × Node 22.12.0/22.22.2. Pushing this branch and CI qualification are authorized. No merge, publication, tag, release, paid evaluation or worktree cleanup is authorized. Do not claim a platform passed until its unit and installed-package jobs finish.
