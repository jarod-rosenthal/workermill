...
PR111 initial heade77d8e20: all six CI jobs (run34155044009) and security checks passed without retry. Review identified bounded edge cases: non-string IDs, corrupt entries consuming list limit, and persisted completion state on resume. Coordinator owns corrections and tests; no merge until corrected head passes qualification. Also align resume hook flag and clarify fresh permission decisions. The stdout any cast was pre-existing moved code, not a resume defect; leave it outside this correction.

Review-correction first local full check:1636 passed/3 failed/one skip. All three new hook assertions incorrectly used expect.anything() for the intentionally undefined optional hooks config; observed WORKERMILL_RESUMED was correct. Corrected to assert undefined explicitly and clear the lifecycle spy between tests. Package/PTY5 passed; typecheck/build passed. Next repeat full validation of this test-only correction.

Corrected local qualification on e77d8e20 plus review diff: npm test exit0,1639 passed/one existing skip,115 files,32.24s. Typecheck/build/diff-check exit0; package/PTY5 passed. Next push bounded correction and qualify PR111 exact new head before merge.

## Session resume feature — implemented

User prioritized dedicated session resume before further monolith work. Checked out/refreshed main31bfd209, created feat/session-resume. Coordinator only. Scope: wm resume picker, --last, explicit ID/unique prefix and --fork in current working-directory session scope; reuse interactive startup and hydration, apply current permission policy with fresh decisions and current provider configuration, reject missing/ambiguous sessions before startup instead of silently creating one. Existing --resume remains. Improve corrupt-entry isolation and atomic session saves as bounded storage support. Resume restores saved conversation/usage; it does not restart in-flight tools/builds. Implemented command/resolver/picker, selected-session hydration, docs and atomic saves; uncommitted on base31bfd209. Focused check failed with a logger ENOENT after temporary test-home removal (45 other tests passed); correcting the new fixture to mock logging. Next verify focused tests and installed CLI/PTY resume before full qualification. No automatic merge/release or unrelated refactors.

Checkpoint 2026-09-07T17:04:15.123554+00:00: base/HEAD31bfd209 plus resume diff, feat/session-resume, coordinator only. Changed index/shared startup, session storage, useAgent/types, resolver/picker, focused/runtime/package tests, README and commands docs. Corrected focused46 passed; full npm test exit0:1638 passed/one existing skip,115 files,31.02s; npm run typecheck/build and git diff --check exit0. Node22.22.2 Linux. Installed-package offline-provider/real-PTY tests5 passed,18.45s, including picker-to-chat transition and saved history in the next model request; missing-ID/nonterminal errors covered. New picker test renamed .test.ts with React.createElement to match repository discovery. Initial failed fixture run remains recorded above. No paid provider calls, merge or release. Implementation1d34b440 pushed successfully to origin/feat/session-resume. Local build includes the feature. No PR or merge created; remote CI has not yet qualified this change. Next integration step is PR/platform qualification when requested; no implementation work remains in this bounded feature.
...
## Browser process boundary — qualified candidate

User said proceed with the ordered monolith-boundary work. Base b59528a5, clean refactor/orchestration-presentation from merged main. Coordinator only. First batch owns browser process cleanup extraction and focused tests, bounded macOS diagnosis, architecture/handoff updates. Existing signal-zero EPERM occurs after SIGKILL in stopProcessGroup. Investigate Darwin zombie-only groups versus real denial; never treat EPERM alone as successful cleanup. Keep TERM/KILL, bounded waiting, profile retention and failure visibility. Qualify this blocker before program/chat extractions. Extracted browser/process-group.ts with independently inspected Darwin process state (bounded ps) and conservative unknown-state handling. Apple XNU kern_sig.c killpg1 excludes zombies then returns EPERM when no signalable member is found; this supports the zombie-only-group diagnosis, which still requires actual macOS CI qualification. First focused run21 passed/2 failed because synthetic-browser helper omitted forwarding the new inspection seam; corrected helper to explicitly represent unknown synthetic group state, retaining original cleanup failure assertions. Corrected focused23 passed. Added five repeated real-orphan checks on each macOS CI job to qualify the intermittent failure without test retries or skipped assertions. Source basis: https://github.com/apple-oss-distributions/xnu/blob/main/bsd/kern/kern_sig.c#L1612 . Local Node22.22.2 validation on b59528a5 plus this diff: full1632 passed/zero failed/one existing skip,113 files,33.31s; typecheck/build/diff-check exit0. Exact implementation dfc8279f1511f8e98f6488b9b3f47c399318875d passed all six jobs in https://github.com/jarod-rosenthal/workermill/actions/runs/34078679707 without retry. Both macOS versions:1631 unit passes/two expected skips,113 files, plus five consecutive orphan-cleanup passes and four package/PTY passes each. Filtered repetitions show17 non-selected cases; these are not new full-suite skips. Typecheck/build and package checks passed across the supported matrix; Ubuntu24 diagnostics passed. This qualifies the presentation and browser-process candidate on refactor/orchestration-presentation; no merge or PR security qualification claimed for this new branch. Main remains fac123c1 with the recorded prior failure until this correction is integrated. Next bounded extraction: program/epic execution service from useOrchestrator.ts, first isolating issue preparation/state and then execution/gates behind typed output callbacks; preserve pause/cancel, retries, usage and terminal outcomes. Chat turn extraction follows separately.

## Monolith boundary iteration — 2026-09-07T02:50:41.419079+00:00

User authorized addressing monolith boundaries and explicitly requested checking out main then cutting a new branch. Completed checkout main, fast-forward from origin/main to merged PR108 commitfac123c15cbad8cad32609ec530287994976e9cb, then created refactor/orchestration-presentation in /home/user/github/workermill. Coordinator only; no agents. First bounded candidate: extract orchestration presentation functions from useOrchestrator into a React-free module with explicit inputs, preserve public exports and all display/state behavior; add focused display-contract coverage where missing and an architecture boundary map. No orchestration flow, models, permissions or cleanup changes. Integration remains blocked by known main macOS browser orphan-cleanup EPERM: CI34077230712, job101605642477; other five jobs and security workflows passed. Prior merge/failure checkpoint preserved on reliability/core4fc195a5 and PR108 description. Completed first isolated extraction: ui/orchestration-presentation.ts owns presentation with a type-only UsageSummary import; hook1326 lines versus1482, helper160 lines. Existing summary exports retained. Added two display-boundary tests and documented next boundaries in docs/architecture.md. Node22.22.2 exact patched lockfile local qualification: focused17 passed (three files); full1627 passed/zero failed/one existing skip,112 files,32.82s; typecheck/build/diff-check exit0. No claim of macOS or new remote CI qualification. Save candidate on refactor/orchestration-presentation; do not integrate or expand until the existing macOS cleanup blocker is addressed. Next boundary after baseline repair: separate program/epic execution from React state, preserving callbacks, gates, retries and cancellation.

## PR qualification blocker

PR108 is open: https://github.com/jarod-rosenthal/workermill/pull/108. Audit34076407778 failed on head91306ee3: fast-uri3.1.5 high-severity advisories (fixed version available within existing range); qs also has moderate advisories. These locked versions also exist in main. The earlier six-job workflow did not run dependency/security scans. Bounded prerequisite: update only affected transitive lockfile entries within existing parent ranges, validate with npm audit and full platform checks, then merge. No audit threshold relaxation, forced major upgrade or unrelated feature changes. Coordinator owns package-lock.json and this handoff. Updated fast-uri3.1.5→3.1.7 and qs6.15.3→6.16.0 within existing parent ranges; npm ci passed (one low development advisory remains). CodeQL check101603357119 also reports seven fixture alerts: browser temp-directory creation (high) and dynamic code interpolation in MCP/root-shell fixtures (six medium). Bounded fixture correction uses mkdtemp, argv transport and fixed scripts relative to their existing workspace; keeps every runtime assertion. No production behavior or scan exclusions changed. On dirty91306ee3 plus this bounded diff: npm ci exit0; npm audit --omit=dev --audit-level=high exit0, zero vulnerabilities; typecheck/build exit0; full suite1625 passed/one existing skip,112 files,33.53s; package/PTY four passed,10.77s; diff-check passed. Next: push fix, wait for every PR108 check including CodeQL summary, merge only the checked head, then verify main checks.
