...
## Three Component Changes

### Component 1: Orchestrator (dispatchMultiStoryPlan)
```typescript
storyBranch: `${planJson.featureBranch}/story-${i}`,
```
Each child task gets a unique branch name in its jiraFields.

### Component 2: ECS Task Runner (runWorkerTask)
```typescript
{
  name: "STORY_BRANCH",
  value:
...
### Component 3: Worker Entrypoint (branch checkout)
```bash
