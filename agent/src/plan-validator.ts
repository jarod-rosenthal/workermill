...
function interpolateFeedbackTemplate(template: string, critic: CriticResult): string {
  ...
  if (critic.risks.length > 0) {
    risksSection = "***REMOVED******REMOVED******REMOVED*** Risks Identified:\n";
  ...
  if (critic.suggestions && critic.suggestions.length > 0) {
    suggestionsSection = "***REMOVED******REMOVED******REMOVED*** Required Changes:\n";
  ...
  if (critic.storyFeedback && critic.storyFeedback.length > 0) {
    storyFeedbackSection = "***REMOVED******REMOVED******REMOVED*** Per-Story Feedback:\n";
...
export function formatRefinementFeedback(critic: CriticResult, plan: ExecutionPlan): string {
  ...
  if (critic.suggestions && critic.suggestions.length > 0) {
    lines.push("***REMOVED******REMOVED******REMOVED*** Suggested Improvements:");
    for (const suggestion of critic.suggestions) {
      lines.push(`- ${suggestion}`);
    }
    lines.push("");
  ...
  if (critic.storyFeedback && critic.storyFeedback.length > 0) {
    lines.push("***REMOVED******REMOVED******REMOVED*** Per-Story Notes:");
    for (const fb of critic.storyFeedback) {
      lines.push(`- **${fb.storyId}**: ${fb.feedback}`);
      if (fb.suggestedChanges) {
        for (const change of fb.suggestedChanges) {
