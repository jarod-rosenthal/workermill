...
`;
    ...
    test("builds prompt with attachments", async () => {
      ...
      const prompt = buildPlanningPrompt({
        ...
        description: task.description || "",
        attachments: [
          { filename: "requirements.md", content: "***REMOVED*** Requirements\n- Feature A\n- Feature B" },
          { filename: "api-spec.yaml", content: "openapi: 3.0.0" },
        ],
      });

      expect(prompt).toContain("***REMOVED******REMOVED*** Attachments");
      expect(prompt).toContain("***REMOVED******REMOVED******REMOVED*** requirements.md");
      expect(prompt).toContain("***REMOVED*** Requirements");
      expect(prompt).toContain("***REMOVED******REMOVED******REMOVED*** api-spec.yaml");
    ...
    test("handles missing optional fields", async () => {
      ...
      expect(prompt).not.toContain("***REMOVED******REMOVED*** Attachments");
    });
  });
