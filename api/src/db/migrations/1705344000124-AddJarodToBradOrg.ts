...
export class AddJarodToBradOrg1705344000124 implements MigrationInterface {
  ...
  public async up(queryRunner: QueryRunner): Promise<void> {
    ...
    const jarod = await queryRunner.query(`
      ...
      WHERE email IN ('admin@localhost', 'jarod@workermill.com', 'jarod.rosenthal@gmail.com', 'jarod@therealjarod.com')
      LIMIT 1
    `);
  ...
  public async down(queryRunner: QueryRunner): Promise<void> {
    ...
    await queryRunner.query(`
      ...
      WHERE org_id = $1
        AND user_id IN (SELECT id FROM users WHERE email IN ('admin@localhost', 'jarod@workermill.com', 'jarod.rosenthal@gmail.com', 'jarod@therealjarod.com'))
    `, [bradOrgId]);
  }
}
