...
export class CreatePlatformOrg1705344000202 implements MigrationInterface {
  ...
  public async up(queryRunner: QueryRunner): Promise<void> {
    ...
    const jarodUsers = await queryRunner.query(`
      SELECT id FROM users WHERE LOWER(email) = 'jarod@oncallshift.com'
    `);
    ...
    if (jarodUsers.length > 0) {
      ...
      // Log the result
      console.log(`Platform org created and jarod@oncallshift.com added as owner`);
    } else {
      console.log(`Warning: jarod@oncallshift.com not found - platform org created without admin`);
    }
  }
