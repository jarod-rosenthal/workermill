  ...
  public async up(queryRunner: QueryRunner): Promise<void> {
    ...
    await queryRunner.query(`
      INSERT INTO kb_labels (org_id, name, color) VALUES
        ('${orgId}', 'Bug', '#ef4444'),
        ('${orgId}', 'Feature', '#3b82f6'),
        ('${orgId}', 'Enhancement', '#22c55e'),
        ('${orgId}', 'Documentation', '#a855f7'),
        ('${orgId}', 'Urgent', '#f97316')
    `);
    ...
    const b2Cols = await queryRunner.query(`
      INSERT INTO kb_columns (board_id, name, position, color, wip_limit) VALUES
        ('${b2}', 'To Do', 0, '#6b7280', NULL),
        ('${b2}', 'In Progress', 1, '#f59e0b', 3),
        ('${b2}', 'Review', 2, '#8b5cf6', 2),
        ('${b2}', 'Done', 3, '#10b981', NULL)
      RETURNING id, position
    `);
    ...
    const b3Cols = await queryRunner.query(`
      INSERT INTO kb_columns (board_id, name, position, color) VALUES
        ('${b3}', 'New', 0, '#ef4444'),
        ('${b3}', 'Triaging', 1, '#f59e0b'),
        ('${b3}', 'In Fix', 2, '#3b82f6'),
        ('${b3}', 'Testing', 3, '#8b5cf6'),
        ('${b3}', 'Resolved', 4, '#10b981')
      RETURNING id, position
    `);
