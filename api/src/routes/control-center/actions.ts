...
import { authenticateRequest, authenticateUser, authenticateApiKey } from "../../middleware/auth.js";
  ...
  asyncHandler(async (req: Request, res: Response) => {
    ...
    await logRepo.save(
      logRepo.create({
        taskId: task.id,
        type: "system",
        message: `[Resume] Task resumed by ${req.user?.id || "API"}. Retry #${newRetryCount}. Skip completed: ${skipCompletedStories}, Reset failed: ${resetFailedStories}`,
        severity: "info",
      })
    );
