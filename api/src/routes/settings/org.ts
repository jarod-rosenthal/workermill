...
/**
 * POST /api/settings/remote-agents/:agentId/disconnect
 * Force-disconnect a running agent (marks offline, clears active tasks)
 */
router.post("/remote-agents/:agentId/disconnect", async (req: Request, res: Response) => {
  try {
    const org = req.organization!;
    const agentId = req.params.agentId as string;
    const agentRepo = AppDataSource.getRepository(RemoteAgent);
    ...
    agent.activeTasks = 0;
    agent.lastHeartbeatAt = new Date(0); // Force stale so heartbeat check treats as offline
    await agentRepo.save(agent);

    logger.info("Remote agent force-disconnected", { agentId, orgId: org.id });
    res.json({ success: true });
  } catch (error) {
    logger.error("Error disconnecting remote agent", { error });
