...
const REQUIRED_SECTIONS = ["***REMOVED******REMOVED*** Role", "***REMOVED******REMOVED*** Guidelines"];
...
function validateMarkdownStructure(content: string): string[] {
  ...
  const headings = lines
    .filter((line) => line.match(/^***REMOVED***{1,6}\s/))
    .map((line) => line.match(/^(***REMOVED***{1,6})\s/)?.[1].length || 0);
