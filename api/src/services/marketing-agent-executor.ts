...
async function buildSystemPrompt(
  ...
  if (personaDirectiveText) {
    sections.push(`***REMOVED******REMOVED*** Persona Directive\n${personaDirectiveText}`);
  }

  // Voice/tone
  if (voiceTone) {
    sections.push(`***REMOVED******REMOVED*** Voice & Tone\n${voiceTone}`);
  }

  // Brand keywords
  if (brandKeywords.length > 0) {
    sections.push(`***REMOVED******REMOVED*** Brand Keywords\n${brandKeywords.join(", ")}`);
  }

  // Budget
  sections.push(`***REMOVED******REMOVED*** Budget Status\n${budgetSummary}`);

  // Campaigns
  sections.push(
    `***REMOVED******REMOVED*** Active Campaigns (${context.activeCampaigns.length})\n${campaignSummaries || "No active campaigns."}`,
  );

  // Recent content
  sections.push(
    `***REMOVED******REMOVED*** Recent Content (last 48h)\n${recentSummaries || "No recent content."}`,
  );

  // Pending reviews
  sections.push(
    `***REMOVED******REMOVED*** Pending Reviews (${context.pendingReviews.length})\n${pendingSummaries || "No pending reviews."}`,
  );

  // Instructions
  sections.push(`***REMOVED******REMOVED*** Instructions
