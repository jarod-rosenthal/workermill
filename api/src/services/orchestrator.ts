...
async function postPrdConstraints(task: WorkerTask, techStack?: TechStack): Promise<void> {
  ...
  if (allConstraints.length === 0 && task.description) {
    ...
    const sectionMatch = description.match(/***REMOVED******REMOVED***?\s*(?:technical?\s*)?constraints?[:\s]*\n([\s\S]*?)(?=\n***REMOVED******REMOVED***|\n\n\n|$)/i);
    if (sectionMatch) {
  ...
  const constraintsContent = techStack
    ? `***REMOVED******REMOVED*** MANDATORY Tech Stack Constraints (from Planning Agent)
...
If you're unsure about a technology choice, ask via the sibling Q&A system.`
    : `***REMOVED******REMOVED*** PRD Technical Constraints
...
async function dispatchMultiStoryPlan(task: WorkerTask): Promise<boolean> {
  ...
  for (let i = 0; i < executionPlan.stories!.length; i++) {
    ...
    if (story.scope) {
      descriptionParts.push(`***REMOVED******REMOVED*** Scope\n${story.scope}`);
    }
    ...
    if (story.estimatedComplexity) {
      descriptionParts.push(`***REMOVED******REMOVED*** Complexity: ${story.estimatedComplexity}`);
    }
    ...
    try {
      ...
      if (!merged) {
        await logTaskEvent(
          parentTask.id,
          "info",
          `🔄 PR ***REMOVED***${child.githubPrNumber} may need update - attempting to sync with base branch...`
        );
        ...
        if (prStatus?.merged) {
          ...
          await logTaskEvent(
            parentTask.id,
            "info",
            `✅ PR ***REMOVED***${child.githubPrNumber} was already merged`
          );
          successCount++;
          continue;
        }
          ...
          if (updateResult.success) {
            ...
            if (conflicts.hasConflicts) {
              await logTaskEvent(
                parentTask.id,
                "info",
                `⚠️ PR ***REMOVED***${child.githubPrNumber} has unresolvable conflicts in ${conflicts.conflictingFiles.length} file(s): ${conflicts.conflictingFiles.slice(0, 3).join(", ")}${conflicts.conflictingFiles.length > 3 ? "..." : ""}`,
                { severity: "warning" }
              );
              conflictCount++;
            }
      ...
      if (merged) {
        ...
        await logTaskEvent(
          parentTask.id,
          "info",
          `⚠️ PR ***REMOVED***${child.githubPrNumber} could not be auto-merged - manual resolution may be required`,
          { severity: "warning" }
        );
...
async function requeueForDeployment(task: WorkerTask): Promise<void> {
  ...
  task.status = "queued";
  task.taskNotes = `DEPLOYMENT_RUN: PR ***REMOVED***${task.githubPrNumber} approved. Deploy and merge.`;
