...
- **targetFiles: max ${input.maxTargetFiles || 5} per story.** List the most important files each story will create or modify, up to ${input.maxTargetFiles || 5} files. If a story needs more than ${input.maxTargetFiles || 5} files, split it into multiple stories. Workers can discover additional files from context — focus targetFiles on the PRIMARY deliverables.
