...
export function incrementToolCount(toolName) {
    toolCounts[toolName] = (toolCounts[toolName] || 0) + 1;
}
export function printHeader(version, provider, model, cwd) {
    console.log();
...
export function getPersonaEmoji(persona) {
 ...
 */
export function wmLogPrefix(persona) {
    const emoji = getPersonaEmoji(persona);
...
function renderTable(lines) {
    ...
    if (dataRows.length === 0)
        return;
    const colWidths = dataRows[0].map((_, colIdx) => Math.max(...dataRows.map(row => (row[colIdx] || "").length)));
...
export function printAgentText(text) {
    if (!text.trim())
        return;
...
function highlightCode(line) {
    ...
    result = result.replace(/(#.*$)/gm, (m) => chalk.dim(m));
    // Keywords
    result = result.replace(/\b(const|let|var|function|return|if|else|for|while|class|import|export|from|async|await|try|catch|throw|new|this|def|self|type|interface)\b/g, (m) => chalk.magenta(m));
    // Types
    result = result.replace(/\b(string|number|boolean|void|null|undefined|true|false|Promise|Array)\b/g, (m) => chalk.yellow(m));
    return result;
}
