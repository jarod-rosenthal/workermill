...
E2E tests (`npm run test:e2e`) run post-push in CI. Workers should also run them locally before pushing — requires `npx playwright install --with-deps chromium` first (browsers are not pre-installed in the worker image).
...
Full-stack Kanban board — Next.js 15, Prisma, PostgreSQL, NextAuth.js v5.

## Quick Reference

| Task | Command |
|------|---------|
| Start database | `docker compose up -d --wait` |
| Install | `npm install` |
...
2. `docker compose up -d --wait` — start local PostgreSQL
3. `npm install`
4. `npx prisma db push` — create tables
5. `npm run db:seed` — populate demo data
...
- Playwright browsers not pre-installed — run `npx playwright install --with-deps chromium` once
```

### Local Development (docker-compose)

This is the **primary development and testing environment**. Workers and human developers both use docker-compose to run a local PostgreSQL. Neon is only for production/CI.
...
**Note:** Playwright browsers are NOT pre-installed. Workers must run `npx playwright install --with-deps chromium` before E2E tests.
...
Playwright tests run against the dev server backed by a real PostgreSQL database (docker-compose locally, Neon in CI).
