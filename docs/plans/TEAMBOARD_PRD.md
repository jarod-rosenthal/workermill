...
## Card 1: Set Up Project Repository and Local Dev Environment
...
#       Note: Card 1 seed.ts should only create the demo user (demo@workermill.com / demo1234).
#       Full seed data (workspaces, boards, cards, activities) is added in Card 2 Phase 1.8.
...
> **Card 1 creates ONLY the files listed above.** Do NOT create:
> - `Dockerfile`, `.dockerignore`, `docker-compose.yml` — Vercel deployment is pre-configured, no Docker needed
> - `vercel.json` — Vercel auto-detects Next.js, no config needed for Card 1 (vercel.json is a Card 6 concern)
...
> - `components.json` — shadcn/ui CLI config is not needed for Card 1 (UI components are Card 3)
...
> **`next.config.js` for Card 1 MUST be minimal:**
> ```js
> /** @type {import('next').NextConfig} */
> const nextConfig = {};
> export default nextConfig;
> ```
> Do NOT add `output: 'standalone'`, `poweredByHeader`, `compress`, `optimizePackageImports`, or `images.formats`. Those are Card 6 Operational Reference items. Card 1 `next.config.js` is an empty config.
...
- [ ] E2E tests are NOT required for Card 1 — auth pages are scaffolds (stubs), E2E is tested properly in Card 3. Create `tests/e2e/` directory and `playwright.config.ts` but no test files yet. The CI E2E job will be a no-op (0 tests to run = pass).

---

## Card 2: Build the Core Backend API
...
## Card 3: Build the Web Dashboard
...
## Card 4: Progressive Web App (PWA)
...
## Card 5: Build Extended Features and Integrations
...
## Worker Execution Rules (MANDATORY — All Cards)
...
**Minimum E2E test coverage (required for each card):**
...
**Every card that adds UI features MUST add corresponding E2E tests.** The health check alone is not sufficient.
...
> **This Operational Reference section is for Card 6 workers.** Card 1 workers should STOP reading at the "Card 1 Definition of Done" section above. The configurations below (vercel.json, performance targets, monitoring) are NOT part of Card 1 scope and should NOT be created during Card 1.

> This section covers production operations: environment setup, deployment, monitoring, troubleshooting, and recovery. Workers executing Card 6 should use this as their primary reference.
