...
| --- | --- |
| Broken orchestration persistence | Integrated saved R15b events and repaired active/terminal finalization, identity preflight, retry lineage, failure/cancellation outcomes, and final UI/result agreement. [R15 acceptance](r15-acceptance.md). |
...
| Superseded runtime code | Reconciled removed tests and restored meaningful MCP transport/emergency coverage. [R23 acceptance](r23-acceptance.md). |
| Runtime contract tests | Added an installed-SDK scripted model and real adapter/registered-tool tests; mounted chat, lifecycle, resume, and child checks. Removed copied helpers with an explicit [coverage map](r16-coverage.md), including which drivers remain mocked. |
| Usage foundation | Call identities, first-observation deduplication, missing/partial/reported usage, unknown/local/known pricing, cache arithmetic, and partial observed costs. Existing rates and model defaults were preserved. |
| Chat and headless accounting | Main/child/compaction calls, interrupted usage, resumed totals, and missing versus reported-zero metadata. |
| Planning, worker and review accounting | Spec check, planning/extraction, critic scoring/refinement, worker/reviewer/revision attempts and children, including failed calls and distinct retries. Cleanup does not wait on a new unresolved usage promise. |
| Run/session/UI accounting | Validated allowlisted run ledger; matching saved/result/output totals; cumulative session model/role totals; progress replay deduplication; old history retained; unknown/incomplete estimates shown. Stats retain historical global tokens when newer model buckets are incomplete. |
...
Additional problems identified during continuation were contained:

- Delegated escalated tool requests stalled without usable results. The coordinator interrupted those waits and moved native checks to root tools. These were coordinator interruptions, not new user cancellations. Worker commits remained candidates until the combined tree passed.
- The pre-existing installed dependency tree differed from package-lock.json: fast-uri 3.1.0 installed versus 3.1.5 locked; Vite 8.0.8 versus 8.2.2. Independent `npm ci` populated the exact graph for qualification while preserving core's original dependencies. Package fixtures also needed correct startup request counting and separate prompt/Enter input events to test active cancellation rather than startup.
- Planning integration failed 91 orchestration tests because the old CostTracker mock lacked recordCall. Removing it made the same assertions use the real tracker. Two later lifecycle mocks had the same defect and were corrected. Failures were checkpointed before repair; downstream work did not proceed while integrated checks were red.
- The multi-role ledger fixture initially retained an old review-token expectation. It was aligned with the new scripted observation. Root review also found child usage still overwrote positive step subtotals with an SDK zero placeholder; the child now uses the shared settlement helper, with partial-positive and genuine-zero regressions.
- Final review found the program-level cancellation gap described above. It was recorded as a bounded lifecycle correction and qualified with mounted gate cancellation and bootstrap request/stream tests.
...
## Remote qualification outcome — 2026-09-07

After the user authorized a branch push, the recovered work was pushed to `reliability/core`. [CI run 34069973409](https://github.com/jarod-rosenthal/workermill/actions/runs/34069973409) qualified `b67160f8393370fc99019b3a5bcff305570e381f` on Ubuntu22.04/macOS26 arm64 and Node22.12.0/22.22.2. Each Linux job passed 1,614 unit tests with one existing skip; each macOS job passed 1,613 with two expected skips. All four package tests, typecheck and build passed in every job. No merge or release occurred.

The initial remote runs exposed failures not visible in local Linux qualification: noncanonical macOS temp paths in fixtures, absolute fixture shell commands rejected by the path guard, a missing executable bit in the native PTY test driver, and CI environment flags suppressing interactive UI rendering. Those were corrected without changing production boundaries or removing assertions. One final macOS checkout had a GitHub runner DNS failure; retrying that job on the same commit passed.

Ubuntu24.04 remains a real compatibility limitation. Loading Ubuntu's packaged bwrap profile made basic namespace startup succeed, but the installed runtime's nested seccomp setup still required capabilities the profile denies. The successful Linux qualification explicitly uses Ubuntu22.04. It does not resolve or conceal the Ubuntu24.04 failure, which remains a documented follow-up in the qualification report and user configuration documentation. No global host protections were disabled.

Command approvals also caused unnecessary idle time, including a nine-minute log-download wait. Shell redirects produced per-command approval rules instead of reusable prefixes. The coordinator switched to explicit read-only `gh api --method GET` requests and saved reusable monitoring/retry permissions. Host permission settings remain user-controlled; no safety-policy bypass was attempted. The [qualification report](r24-qualification.md) retains failed-run IDs, exact corrections and remaining limits.
