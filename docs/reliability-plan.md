...
Mandatory dispatches: R11r first adds run-owned MCP/LSP acquisition and awaited teardown, with focused resource tests and the narrow registry signature needed for callers. Own only `mcp-client`, `engine/tools/lsp`, that registry signature, and resource tests: no caller migration in this dispatch. Prevent a second run from overwriting or closing another run's server; partial startup and pending requests must settle on cancellation. Preserve CLI-exit cleanup separately. Global `stopAllMCPServers`/`lsp.shutdown` are not run-scoped APIs.

R11a then integrates chat/headless cancellation and tests (`useAgent`, `run-command`) using R11r. It also replaces `Root`'s blocking direct `!command` execution with the shared async process lifecycle, preserving explicit user-command authorization and making cancellation/exit settle its process group; do not route direct user commands through an LLM or infer blanket future tool approval. R11b integrates orchestration/roles/children, `useOrchestrator`, and stage cancellation tests. R11b depends on the integrated R11a lifecycle contract.
