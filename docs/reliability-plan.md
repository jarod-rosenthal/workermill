...
- A task with `dispatches` in the queue is an aggregate, not a worker assignment. Run its numbered suffix packages in the listed order, one worker per package; mark the parent complete only when all packages are integrated. A dispatch inherits parent locks unless the queue gives it an explicit narrower `locks` list matching its documented write scope. Exact file ownership still cannot overlap. Each dispatch receives the parent acceptance contract but only its specified write scope. This makes the larger tasks concrete without asking a smaller model to invent its own decomposition.
...
Mandatory resource dispatches: R11r first adds run-owned MCP acquisition, cancellable requests, and awaited teardown in `mcp-client` plus focused resource tests. R11s then adds the corresponding LSP lifetime in `engine/tools/lsp`, its narrow registry binding, and focused resource tests. No caller migration in these resource dispatches. Prevent a second run from overwriting or closing another run's server; partial startup and pending requests must settle on cancellation. Preserve CLI-exit cleanup separately. Global `stopAllMCPServers`/`lsp.shutdown` are not run-scoped APIs. These are ownership/cancellation changes, not claims that server subprocesses run inside the OS sandbox.

R11a then integrates chat/headless cancellation and tests (`useAgent`, `run-command`) using R11r/R11s. It also replaces `Root`'s blocking direct `!command` execution with the shared async process lifecycle, preserving explicit user-command authorization and making cancellation/exit settle its process group; do not route direct user commands through an LLM or infer blanket future tool approval. R11b integrates orchestration/roles/children, `useOrchestrator`, and stage cancellation tests. R11b depends on the integrated R11a lifecycle contract.

R11t addresses the separate browser lifetime discovered in `src/browser.ts`, after R11a. Own `browser.ts`, narrow browser bindings in `useAgent`, `/browser` handling only where required, and browser runtime tests; locks `browser-runtime`, `interactive-runtime`. It can run alongside R11b, which must not edit `useAgent`. Replace global model-tool Chrome/CDP ownership with a run-owned instance and signal-aware startup, HTTP/WebSocket requests, and awaited bounded teardown. Reject every pending CDP request on close; stop the owned process group even after its parent exits; bound response/console buffers. Use a private temporary browser profile and do not attach to an unrelated process's debugging port or reuse the user's existing browser profile. Preserve explicit `/browser` session controls as a separate owner rather than letting one model turn close another owner. Keep exported tool names and ordinary browser behavior; no new provider dependencies or live Chrome downloads. Tests use a dependency-free fake CDP/Chrome process for started-request/startup cancellation, disconnect, failed startup, concurrent owner separation, and profile cleanup. Native Windows browser/process-group behavior requires a separately qualified path; do not promise Unix-group cleanup for a Windows executable launched through WSL. This is resource ownership, not a claim that Chrome is inside the tool OS sandbox.

Adapter acceptance must cover a model stream rejecting after starting a tool: abort the owned attempt, await dispatched tools and owned processes, then return failure. Do not swallow arbitrary stream errors as expected cancellation or infer success from an independently resolved `stream.text`. Retry delays must observe the run signal, and cancellation must prevent the next model call. Cleanup attempts every owned resource even if an earlier cleanup fails; teardown failures are visible non-success outcomes. Only CLI-exit cleanup may close resources belonging to other runs.

R11u closes the HTTP-tool lifetime gap: own `engine/http-request.ts`, `engine/tools/{fetch,download-file,web-search,ticket}.ts`, `ticket-ops.ts`, and focused local HTTP fixtures; lock `network-tools`, after R11a. It may run alongside R11b/R11t with R11b owning the four narrow registry signal bindings and orchestration TicketOps constructor binding. Deadlines cover headers and response bodies; responses are bounded before parsing. Downloads stream under a fixed size/time bound into a unique owned temporary file, preserve an existing destination on failure/cancellation, and replace it only after completion. Ticket operations snapshot their credential/repository context rather than mutating process-wide variables; discovery probes use the bounded process runner. Cancellation prevents later calls in multi-request operations. Model-directed remote-write failures must not report success; optional orchestration notifications may remain visibly best-effort. Qualify started-body cancellation, independent requests, timeout after headers, oversize response, failed-download preservation, and cancellation before a second ticket mutation using local HTTP fixtures with dummy credentials. No live tracker requests are needed.
...
R13b fingerprints HEAD, index entries (object ID and mode), and current tracked/untracked non-ignored file bytes, modes, and deletions. Hash symlink targets without following them outside the repository; do not execute Git filters, external diff tools, hooks, or filesystem monitors to calculate evidence. Same HEAD with changed index or working files is different evidence. If a repository feature such as a dirty submodule cannot be represented safely, return explicitly unverified rather than silently omitting it.

Structured review approval requires both an explicit approved decision and the configured score threshold. A high score must not promote a rejected or revision-needed decision to approval. Invalid or contradictory markers, absent reviewer, exhausted revisions, timeout, and cancellation remain distinct non-approval outcomes. Preserve the configured model, threshold, and revision limit while making this interpretation consistent across standalone and inline review.

R13c separates candidate preparation from publication. Any final commit or source-changing cleanup belongs before final evidence is captured. Verify the candidate, review it, and compare fingerprints after every phase; reviewer revisions or source-mutating gates invalidate prior evidence. Retry only a bounded number of stabilization cycles, then block with inspectable local work if evidence cannot stabilize. Immediately before publishing, require passing blocking gates and the applicable structured review outcome for that exact state. Disabled review is distinct from approval. Keep `ship_complete` semantics honest: do not emit completion before publication merely to move its hooks earlier. Source-changing completion hooks must not cause unverified code to be published or reported as verified. Test both unchanged-state reuse and invalidation without making real push/PR/ticket calls.

Truncated-review diff files are temporary inputs, not deliverables. Give each one a unique owned path, clean it in `finally` before revision workers or final fingerprint collection, and never overwrite or delete a pre-existing user file at a fixed name. Do not solve artifact-induced fingerprint drift by excluding arbitrary untracked files from evidence. Include a large-diff regression proving the artifact is readable during review but absent before commit/publication, including a failed review.
...
R15a uses the explicit `manifest-storage` and `run-readers` locks and writes only `run-manifest.ts`, `runs-command.ts`, `recovery.ts`, and their tests. It can run beside R23's removal of unused runtime code. R15b retains the parent adapter locks; do not dispatch it until R23 has integrated.

R15a's storage contract must distinguish an active record from a terminal result; creation must not label a running build cancelled. Retain the existing numeric summary fields for existing callers, but expose explicit version, phase, terminal reason, and optional prior-run ID. Store story attempts separately from planned stories so resume does not invent new attempts for completed work. Use typed gate/review evidence from R13 rather than reparsing display text. Validate loaded JSON before rendering; skip corrupt/unsupported records without crashing `wm runs`. Read the existing unversioned format with a visible legacy-evidence limitation, not a fabricated verified result. Validate run IDs before using them as paths; write a unique same-directory temporary file with exclusive creation and restrictive permissions, then atomically rename. A failed replacement must leave the prior record readable and report persistence failure to the caller. Test interrupted replacement, malicious IDs, malformed records, ordering, ambiguous prefixes, and legacy records with real temporary storage. Do not persist whole provider configuration, credentials, prompts, tool payloads, or gate output merely to populate a report. Recovery must distinguish a missing branch from a failed branch probe; a timeout is not proof the branch was deleted.
...
The scripted fixture should implement the installed SDK's model interface (for example `MockLanguageModelV3` from `ai/test`), leaving real `streamText`/tool dispatch in the path. The existing `helpers/mock-ai-sdk.ts` merely returns text and step callbacks; it cannot prove SDK tool execution. Cover at least one tool request followed by its result in the next model prompt and a normal final answer. Treat unexpected extra model calls as test failures, bind abort during an already-started stream, and give scripted calls stable IDs for R18. Mounted Ink hooks are testable with in-memory streams; replace the copied permission/compaction/output implementations in `useAgent.test.ts` and `useOrchestrator.test.ts` with actual hook assertions or existing production exports, not another helper containing mirrored logic.
...
Add offline packaged-entry tests for `--help`, `--version`, headless JSON/exit status, and cancellation using the scripted provider transport. Install the packed artifact into a temporary directory when checking shipped assets/dependencies. Add Linux/macOS coverage for Node 22.12 lower bound and the existing Node 22.22.2 baseline; the installed Ink 7 and Vite 8 dependencies exclude Node 20 and earlier Node 22 minors from the supported development matrix. Read actual lockfile/package engines before extending support. Native Windows shell support is not added by this ticket; document WSL boundaries. Put genuinely OS-dependent tests in a named deterministic suite and run it on supported CI hosts.
...
R18c dispatch refinement (2026-09-06): R18c1 owns planning/spec-check/critic/extraction usage callbacks plus their orchestration call sites; R18c2 owns workers, standalone and orchestration reviews, revisions and child callbacks; R18c3 owns version-compatible manifest/session/UI ledger summaries and whole-run sum qualification. Execute sequentially. Each actually started SDK invocation has one fresh identity, including fallback/retry calls; final reported usage and partial completed-step usage are distinct. Remove old aggregate addUsage calls only as the corresponding actual call observations are wired. Keep planning/review/tool policy and routing unchanged. Auxiliary classification/bootstrap calls outside the run must be documented explicitly if they remain outside the run ledger; do not claim application-wide accounting from orchestration-only coverage.

R18c3 is further split before dispatch because persistence and session/UI migration are distinct contracts: c3a adds a bounded, allowlisted, validated optional ledger to the current manifest version (old records remain readable), returns run observations and emits an optional output ledger callback; c3b applies new call IDs once to cumulative session totals/model/role summaries and renders unknown/incomplete estimates in run/session UI. A session may contain historical totals with no historical call ledger; preserve those totals and label that limitation rather than fabricate observations. Run totals are per-run; session totals remain cumulative. Whole-run sum tests qualify the completed adapter/storage contract in c3a, with mounted/session agreement tests in c3b.
