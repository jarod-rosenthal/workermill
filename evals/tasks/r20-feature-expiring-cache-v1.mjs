...
export const fixture = {
  taskId: "r20-feature-expiring-cache-v1", category: "feature", initialRevision: initialRevision(base),
...
async function accepts(root, mainUrl, timeoutMs) {
  return runNode(root, `import { createSessionCache } from ${JSON.stringify(mainUrl)};
let time = 100; const cache = createSessionCache(() => time); cache.set("token", "ok", 50);
if (cache.get("token") !== "ok") process.exit(3); time = 150;
if (cache.get("token") !== undefined) process.exit(3); time = 125;
if (cache.get("token") !== undefined) process.exit(3);
cache.set("token", "renewed", 10); time = 134;
if (cache.get("token") !== "renewed") process.exit(3); time = 135;
if (cache.get("token") !== undefined) process.exit(3);`, timeoutMs);
}
