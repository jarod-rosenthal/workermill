...
export default function BoardView() {
  ...
  const [issueTrackerProvider, setIssueTrackerProvider] = useState<string>("internal");

  useEffect(() => {
    apiClient.get("/settings").then((res) => {
      setIssueTrackerProvider(res.data?.issueTrackerProvider || "internal");
    }).catch(() => {});
  }, []);
      ...
      <header className="border-b border-border/30 glass-strong sticky top-0 z-10">
        <div className="max-w-full mx-auto px-6 py-3 flex items-center justify-between">
          ...
          <div className="flex items-center gap-2">
            {issueTrackerProvider === "github-issues" && currentBoard.githubRepo && (
              <a
                href={`https://github.com/${currentBoard.githubRepo}/issues?q=label%3Aworkermill`}
                target="_blank"
