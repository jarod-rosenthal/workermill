...
function DepthGrid() {
  ...
  return (
    <group ref={gridRef} position={[0, -8, -20]} rotation={[Math.PI * -0.4, 0, 0]}>
      <gridHelper args={[240, 40, "***REMOVED***14b8a6", "***REMOVED***14b8a6"]} />
      <mesh position={[0, -0.1, 0]} rotation={[Math.PI / 2, 0, 0]}>
        <planeGeometry args={[240, 240]} />
        <meshBasicMaterial color="***REMOVED***14b8a6" transparent opacity={0.02} side={THREE.DoubleSide} />
      </mesh>
    </group>
  )
}
  ...
  return (
    <group ref={meshRef} position={position} rotation={rotation}>
      ...
      <mesh>
        ...
        <meshBasicMaterial color="***REMOVED***14b8a6" transparent opacity={opacity} side={THREE.DoubleSide} />
      </mesh>
      {/* Panel border glow */}
      <mesh position={[0, 0, -0.01]}>
        <planeGeometry args={[size[0] + 0.05, size[1] + 0.05]} />
        <meshBasicMaterial color="***REMOVED***14b8a6" transparent opacity={opacity * 0.5} side={THREE.DoubleSide} />
      </mesh>
      {/* Subtle line details */}
      <mesh position={[0, size[1] * 0.35, 0.01]}>
        <planeGeometry args={[size[0] * 0.8, 0.02]} />
        <meshBasicMaterial color="***REMOVED***22d3ee" transparent opacity={opacity * 2} />
      </mesh>
      <mesh position={[0, size[1] * 0.15, 0.01]}>
        <planeGeometry args={[size[0] * 0.6, 0.015]} />
        <meshBasicMaterial color="***REMOVED***14b8a6" transparent opacity={opacity * 1.5} />
      </mesh>
...
function Scene() {
  return (
    <>
      <color attach="background" args={["***REMOVED***030608"]} />
      <fog attach="fog" args={["***REMOVED***030608", 5, 50]} />
      ...
      <NebulaCloud position={[15, 5, -35]} scale={8} color="***REMOVED***14b8a6" opacity={0.03} shapeIndex={0} />
      <NebulaCloud position={[-18, -3, -40]} scale={10} color="***REMOVED***3b82f6" opacity={0.025} shapeIndex={1} />
      <NebulaCloud position={[5, -8, -45]} scale={12} color="***REMOVED***8b5cf6" opacity={0.02} shapeIndex={2} />
      <NebulaCloud position={[-10, 8, -30]} scale={6} color="***REMOVED***0891b2" opacity={0.035} shapeIndex={3} />

      {/* Atmospheric glows - large soft light sources */}
      <AtmosphericGlow position={[12, 4, -20]} scale={5} color="***REMOVED***14b8a6" intensity={1.2} />
      <AtmosphericGlow position={[-10, -3, -25]} scale={6} color="***REMOVED***3b82f6" intensity={0.8} />
      <AtmosphericGlow position={[0, 8, -30]} scale={4} color="***REMOVED***8b5cf6" intensity={0.6} />
      <AtmosphericGlow position={[-15, 5, -18]} scale={3} color="***REMOVED***0891b2" intensity={0.7} />

      {/* Perspective grid floor */}
      <DepthGrid />

      {/* Glass orbs at mid-depth - slow, subtle movement */}
      <GlassOrb position={[8, -2, -10]} scale={2.4} color="***REMOVED***14b8a6" speed={0.3} />
      <GlassOrb position={[-6, -3, -12]} scale={3.0} color="***REMOVED***3b82f6" speed={0.25} />
      <GlassOrb position={[4, -5, -8]} scale={1.6} color="***REMOVED***22d3ee" speed={0.35} />
      <GlassOrb position={[-4, -1, -14]} scale={2.0} color="***REMOVED***8b5cf6" speed={0.2} />
      <GlassOrb position={[10, -6, -16]} scale={1.2} color="***REMOVED***14b8a6" speed={0.4} />
      ...
      <FogLayer z={-35} opacity={0.15} color="***REMOVED***030608" />
      <FogLayer z={-45} opacity={0.25} color="***REMOVED***030608" />
...
export function ImmersiveBackground() {
  ...
  return (
    <div className="fixed inset-0">
      ...
      {isMobile ? (
        <div className="absolute inset-0 bg-[***REMOVED***030608]">
      ...
      {!isMobile && (
        <div className="absolute inset-0 pointer-events-none">
          ...
          <div className="absolute top-0 left-0 right-0 h-40 bg-gradient-to-b from-[***REMOVED***030608] via-[***REMOVED***030608]/70 to-transparent" />
          {/* Left gradient for text content */}
          <div className="absolute top-0 left-0 bottom-0 w-[50%] bg-gradient-to-r from-[***REMOVED***030608]/95 via-[***REMOVED***030608]/60 to-transparent" />
          {/* Bottom gradient */}
          <div className="absolute bottom-0 left-0 right-0 h-64 bg-gradient-to-t from-[***REMOVED***030608] via-[***REMOVED***030608]/50 to-transparent" />
          {/* Vignette */}
          <div className="absolute inset-0 bg-[radial-gradient(ellipse_at_center,transparent_0%,***REMOVED***030608_85%)] opacity-40" />
        </div>
      )}
