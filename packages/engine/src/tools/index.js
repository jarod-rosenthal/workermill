 ...
 */
export function createToolDefinitions(workingDir, model, sandboxed = true) {
    return {
        bash: tool({
        ...
        read_file: tool({
            ...
            inputSchema: z.object({
                ...
                maxLines: z
                    .number()
                    .optional()
                    .describe("Maximum number of lines to read (optional, reads entire file if not specified)"),
                startLine: z
                    .number()
                    .optional()
        ...
        edit_file: tool({
            ...
            inputSchema: z.object({
                ...
                new_string: z
                    .string()
                    .describe("The text to replace old_string with. Can be empty string to delete."),
                replaceAll: z
                    .boolean()
                    .optional()
                    .describe("Replace all occurrences instead of requiring unique match (default: false)"),
            }),
        ...
        grep: tool({
            ...
            inputSchema: z.object({
                ...
                contextLines: z
                    .number()
                    .optional()
                    .describe("Number of context lines before and after match (default: 0)"),
                maxResults: z
                    .number()
                    .optional()
                    .describe("Maximum number of total matches to return (default: 100)"),
            }),
                        ...
                        const readOnlyTools = {
                            ...
                            grep: tool({
                                ...
                                execute: async ({ pattern, path: searchPath, filePattern }) => {
                                    ...
                                    const lines = [`Found ${result.matchCount} match(es):`];
                                    for (const [file, matches] of Object.entries(result.results)) {
                                        for (const match of matches) {
                                            lines.push(`${file}:${match.line}: ${match.content}`);
                                        }
