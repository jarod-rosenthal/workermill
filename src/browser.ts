...
export interface BrowserRunOptions {
  ...
  inspectProcessGroup?: (groupId: number) => Promise<number[] | null>;
}
...
export function createBrowserRunResources(options: BrowserRunOptions): BrowserRunResources {
  ...
  const cleanupInternal = async (): Promise<void> => {
    ...
    cleanup = (async () => {
      ...
      try {
        if (owned?.pid) await stopProcessGroup(owned, killProcess, terminationGraceMs, options.inspectProcessGroup);
        processStopped = true;
