...
function isLongRunning(command: string): string | null {
  for (const pattern of LONG_RUNNING_PATTERNS) {
    if (pattern.test(command)) return command.match(pattern)?.[0] ?? "long-running process";
  }
  return null;
}
...
export function commandUsesDocker(command: string): boolean {
  return command.split(/&&|\|\||;|\n/).some((segment) => {
    return /^(?:[A-Z_][A-Z0-9_]*=(?:"[^"]*"|'[^']*'|[^\s]+)\s+)*(?:docker|docker-compose)\b/i.test(segment.trim());
  });
}
...
export async function execute({
  ...
  if (longRunning) {
    return {
      ...
      stdout: "",
      stderr: "",
      error: `Blocked: "${longRunning}" is a long-running process that would block execution. Use a one-shot command instead (for example, \"npx tsc --noEmit\" or \"npm test\").`,
      duration: 0,
    };
  }
  ...
  if (result.reason === "spawn_failed") {
    return { success: false, exitCode: null, stdout: visibleStdout, stderr, error: `Failed to execute command: ${stderr}`, duration };
  }
  if (result.reason === "timed_out") {
    return { success: false, exitCode: result.exitCode, stdout: visibleStdout, stderr, error: `Command timed out after ${timeout}ms`, duration };
  }
