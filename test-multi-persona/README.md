***REMOVED*** Multi-Persona Single Container Test Harness
...
***REMOVED*** Dry run - shows prompts without executing Claude (free, good for testing setup)
./run-test.sh --dry-run

***REMOVED*** Full run - executes Claude for each subtask (costs API credits)
./run-test.sh
```
...
***REMOVED******REMOVED*** Customizing the Test
...
```
test-multi-persona/
├── context-store.json     ***REMOVED*** Full conversation between personas
├── test-repo/
│   ├── .git/
│   ├── package.json
│   ├── README.md
│   └── src/
│       ├── api.js         ***REMOVED*** Created by backend_developer
│       ├── Greeting.jsx   ***REMOVED*** Created by frontend_developer
│       └── api.test.js    ***REMOVED*** Created by qa_engineer
...
```bash
***REMOVED*** Remove all generated files
