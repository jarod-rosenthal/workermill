#!/bin/bash
# helpers.sh - File-based mock of WorkerContext API
# These functions mimic the real API but use a local JSON file
...
# Fetch all context messages (mimics GET /api/coordination/context/:parentTaskId)
fetch_context() {
    init_context_store
...
# Post a context message (mimics POST /api/coordination/context)
post_context() {
    local type="$1"
    local content="$2"
...
# Display current context store (for debugging)
show_context() {
