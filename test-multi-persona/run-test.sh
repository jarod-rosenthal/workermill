#!/bin/bash
# run-test.sh - Multi-Persona Single Container Test Harness
#
# This script proves out the core concept:
# 1. Sequential subtask execution with different personas
# 2. Fresh Claude Code context per subtask
# 3. Context handoff via file-based mock of WorkerContext API
# 4. Git commits per subtask
# 5. Transactional subtasks with retry/rollback on failure
#
# Usage: ./run-test.sh [--dry-run]
...
# Check for dry-run mode (shows prompts without running Claude)
DRY_RUN=false
if [ "$1" == "--dry-run" ]; then
    DRY_RUN=true
...
# Reset context store for fresh test
reset_context() {
...
# Get current context count for a persona
...
# Parse context markers from output and post them
# Returns 0 if at least one ::context::completion:: marker was found
parse_and_post_context_markers() {
    ...
    # Extract all ::context:: markers from the output
    while IFS= read -r line; do
        # Match ::context::<type>::<content>
        if [[ "$line" =~ ::context::([a-z_]+)::(.+) ]]; then
            local msg_type="${BASH_REMATCH[1]}"
            local content="${BASH_REMATCH[2]}"

            # Clean up content (remove trailing whitespace, quotes, backticks)
...
# Rollback git state to a specific commit
rollback_to_commit() {
    local commit="$1"
    local persona="$2"
...
execute_subtask_with_retry() {
    ...
    # ==========================================
    # STEP A: Snapshot current state (Safety Net)
    # ==========================================
    cd "$TEST_REPO"
    ...
    while [ $attempt -le $MAX_RETRIES ]; do
        ...
        # Fetch context from previous subtasks
        ...
        if [ "$DRY_RUN" == "true" ]; then
            ...
            # Simulate output with context markers for dry-run
...
EOF
        ...
        # Use tee to stream output in real-time while also saving to file
        echo ""
        ...
        # ==========================================
        # STEP C: Parse Output Markers & Validate
        # ==========================================
        # Parse ::context:: markers from output and post them to context store
        # This matches production behavior where shell parses markers, not Claude calling bash
        ...
        # ==========================================
        # STEP D: Decision - Success or Rollback
        # ==========================================
        if [ $claude_exit_code -eq 0 ] && [ "$markers_valid" == "true" ]; then
            ...
            # Copy final output to standard location
            cp "$output_file" "${TEST_DIR}/output-${index}-${persona}.txt"

            success=true
            break
        else
            # FAILURE - Rollback and possibly retry
    ...
    if [ "$success" == "true" ]; then
        ...
        # Post blocker to context for debugging
...
main() {
    ...
    # Post initial constraints (like orchestrator would)
    ...
    # Execute each subtask with retry/rollback
    for i in $(seq 0 $((subtask_count - 1))); do
        ...
        # Small delay between subtasks for readability
        sleep 1
    done
