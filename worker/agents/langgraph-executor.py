...
# Supported providers and their default models (updated 2024-01)
PROVIDER_DEFAULTS = {
    "ollama": "qwen2.5-coder:32b",
    "openai": "gpt-5.1-codex",
    "gemini": "gemini-3-pro-preview",
    "google": "gemini-3-pro-preview",  # Alias for gemini
...
class AgentState(TypedDict):
    ...
    test_results: dict[str, TestResult]  # command -> {success, output, timestamp}
    modified_files: set[str]  # Files that have been written/edited
    iteration: int  # Current iteration count
    files_modified_since_test: bool  # Track if cache is stale
...
# =============================================================================
# State Manager (Singleton for tool access)
# =============================================================================
...
class StateManager:
    ...
    edit_failures: dict[str, int] = field(default_factory=dict)  # filepath -> failure count
    recent_bash_commands: list[str] = field(default_factory=list)  # Track recent bash commands for loop detection
    file_contents_cache: dict[str, str] = field(default_factory=dict)  # Cache file contents for edit guidance
    last_failed_edit: dict[str, dict] = field(default_factory=dict)  # Track last failed edit params per file
    ...
    def record_edit_failure(self, filepath: str, old_string: str = "", new_string: str = "") -> int:
        ...
        # Store the failed edit params for guidance
        self.last_failed_edit[filepath] = {"old_string": old_string, "new_string": new_string}
        log(f"[edit] Failed edit attempt #{self.edit_failures[filepath]} for: {filepath}")
    ...
    def get_edit_guidance(self, filepath: str, failure_count: int) -> str:
        ...
        if failure_count == 1:
            ...
            # Provide more specific guidance with the cached content
            content_preview = ""
            if cached_content:
                # Show first 500 chars of the file to help model find the right location
            ...
            # Failure 3+: Strongly push toward write_file
            return (
    ...
    def _commands_similar(self, cmd1: str, cmd2: str) -> bool:
        ...
        # Check if they're calling the same script
...
# Global state manager (tools need access)
...
def bash(command: str, timeout: int = 120000) -> str:
    ...
    """
    max_timeout = 600000
    actual_timeout = min(timeout, max_timeout) / 1000  # Convert to seconds
...
def read_file(path: str, offset: int | None = None, limit: int | None = None) -> str:
    ...
    try:
        ...
        # Cache full file content for edit guidance (before applying offset/limit)
...
def glob(pattern: str, path: str | None = None) -> str:
    ...
    try:
        ...
        # Determine the appropriate find command based on pattern structure
        if "**" in pattern:
            # Recursive search pattern like "src/**/*.ts" or "**/*.md"
            parts = pattern.split("**")
            path_prefix = parts[0].rstrip("/")  # e.g., "src" from "src/**/*.ts"
...
4. **write_file** - Create/overwrite files
   ...
   - Example: write_file(path="/workspace/repo/docs/audit.md", content="# Report...")
...
def state_modifier(state: AgentState) -> list[BaseMessage]:
    ...
    # Add iteration warning if getting close to limit
...
def create_agent(provider: str, model_name: str, system_prompt: str | None = None):
    ...
    # Use messages_modifier for system prompt injection instead
...
def run_agent(
    ...
    # Create initial state (LangGraph v1.0+ uses simple messages dict)
    state = {
        "messages": messages,
    }
...
# =============================================================================
# Alternative: Manual ReAct Loop (for more control)
# =============================================================================
    ...
    while iteration < MAX_ITERATIONS:
        ...
        try:
            ...
            if hasattr(response, "tool_calls") and response.tool_calls:
                    ...
                    if tool_fn:
                        try:
                            ...
                            if tool_name == "edit_file":
                                ...
                                # Check if we should block this edit (too many failures)
                                current_failures = state_manager.edit_failures.get(filepath, 0)
                                if current_failures >= 3:
                                    # Block the edit and force alternative action
                                    block_msg = (
                                ...
                                if "[Failed]" in result and "Could not find" in result:
                                    ...
                                    print(f"[System] Edit failure #{failure_count} - injecting guidance\n")
                                    # Inject guidance after the tool result
                                    messages.append(
                                        ToolMessage(content=result, tool_call_id=tool_id)
                                    )
                                    messages.append(
                ...
                # Check for planning language (not done yet)
                planning_phrases = [
                    "let me",
                    "i'll ",
                    "i will",
                ...
                if any(phrase in content_lower for phrase in planning_phrases):
                    # Model is planning - prompt it to take action
                    ...
                    # No tool calls and not planning - probably done
