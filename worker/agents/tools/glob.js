...
module.exports = {
  name: 'glob',
  description: 'Find files matching a glob pattern. Supports patterns like "**/*.ts", "src/**/*.js", "*.{ts,tsx}". Excludes node_modules and hidden files by default.',
  parameters: {
    type: 'object',
    properties: {
      pattern: {
  ...
  async execute({ pattern, cwd, maxResults = 1000, includeHidden = false }) {
    try {
      const searchDir = cwd
