...
module.exports = {
  name: 'grep',
  description: 'Search for a pattern in files. Uses regex pattern matching. Returns matching lines with file paths and line numbers.',
  parameters: {
    type: 'object',
    properties: {
      pattern: {
  ...
  async execute({ pattern, path: searchPath, filePattern, ignoreCase = false, contextLines = 0, maxResults = 100 }) {
    try {
      ...
      for (const file of filesToSearch) {
        ...
        if (matches.length > 0) {
          const relativePath = path.relative(targetPath, file).replace(/\\/g, '/') || path.basename(file);
