...
***REMOVED******REMOVED*** Go Testing

When working on Go projects (detected by `go.mod`), follow these patterns:

***REMOVED******REMOVED******REMOVED*** Running Go Tests

```bash
***REMOVED*** All tests with verbose output
go test ./... -v -count=1

***REMOVED*** With race detector (catches concurrent access bugs)
...
***REMOVED*** Specific package
go test ./internal/services/... -v -run TestEvaluation
```

***REMOVED******REMOVED******REMOVED*** Go Test Patterns with testify
...
***REMOVED******REMOVED******REMOVED*** Table-Driven Tests (Go idiom)
...
***REMOVED******REMOVED******REMOVED*** Statistical Tests (Rollout Uniformity)
...
***REMOVED******REMOVED******REMOVED*** Go Test Quality Checklist
