...
***REMOVED*** Token tracking uses log-parser.cjs (same as oncallshift)
...
***REMOVED*** 1. Pass through all output to stdout
***REMOVED*** 2. Extract token usage using Math.max() accumulation
***REMOVED*** 3. Send usage to /api/tasks/:id/usage endpoint at end
claude \
    --print \
    --verbose \
...
***REMOVED*** Note: Output may contain JSON stream data after the marker value, so we extract only the clean value
if grep -q "::pr_url::" "${OUTPUT_FILE}"; then
    ***REMOVED*** Extract URL - stops at first quote, whitespace, or end of line
    PR_URL=$(grep "::pr_url::" "${OUTPUT_FILE}" | head -1 | sed 's/.*::pr_url:://' | sed 's/["'"'"'\\ ].*//' | tr -d '\r\n')
    echo "::pr_url::${PR_URL}"
fi

if grep -q "::pr_number::" "${OUTPUT_FILE}"; then
    ***REMOVED*** Extract only digits for PR number
    PR_NUMBER=$(grep "::pr_number::" "${OUTPUT_FILE}" | head -1 | sed 's/.*::pr_number:://' | grep -o '^[0-9]*')
    echo "::pr_number::${PR_NUMBER}"
fi

if grep -q "::branch::" "${OUTPUT_FILE}"; then
    ***REMOVED*** Extract branch name - stops at first quote, whitespace, or invalid char
...
***REMOVED*** Token usage is already sent by log-parser.cjs to /api/tasks/:id/usage
***REMOVED*** We just need to report task completion with result and PR info
...
if [ -n "${API_BASE_URL}" ] && [ -n "${ORG_API_KEY}" ]; then
    ...
    ***REMOVED*** Note: Token data NOT included - already sent by log-parser.cjs to /usage endpoint
    PR_URL_JSON="null"
    PR_NUMBER_JSON="null"
    BRANCH_JSON="null"
