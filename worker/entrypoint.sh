...
***REMOVED*** Fetch GitHub PR feedback (reviews and comments)
***REMOVED*** Called after repo clone to get external feedback on existing PRs
***REMOVED*** Writes feedback to /tmp/github_pr_feedback.md
fetch_github_pr_feedback() {
    ...
    ***REMOVED*** Check if there's an existing PR for this branch
    local pr_info
    ...
    echo "[github] Found existing PR ***REMOVED***${pr_number} - fetching feedback..."
    EXISTING_PR_NUMBER="${pr_number}"

    ***REMOVED*** Start building feedback file
    echo "***REMOVED*** GitHub PR Feedback" > /tmp/github_pr_feedback.md
    echo "" >> /tmp/github_pr_feedback.md
    echo "**PR ***REMOVED***${pr_number}**" >> /tmp/github_pr_feedback.md
    echo "" >> /tmp/github_pr_feedback.md

    ***REMOVED*** Fetch PR reviews (approvals, change requests, comments)
    local reviews
    reviews=$(gh pr view "${pr_number}" --json reviews -q '.reviews[] | select(.state != "COMMENTED") | "***REMOVED******REMOVED******REMOVED*** Review by \(.author.login) - \(.state)\n\(.body)\n---"' 2>/dev/null)

    if [ -n "${reviews}" ]; then
        echo "***REMOVED******REMOVED*** Code Reviews" >> /tmp/github_pr_feedback.md
        echo "" >> /tmp/github_pr_feedback.md
    ...
    ***REMOVED*** Fetch review comments (inline code comments)
    local review_comments
    review_comments=$(gh api "repos/${GITHUB_REPO}/pulls/${pr_number}/comments" --jq '.[] | "***REMOVED******REMOVED******REMOVED*** \(.user.login) on \(.path):\(.line // .original_line)\n\(.body)\n---"' 2>/dev/null)

    if [ -n "${review_comments}" ]; then
        echo "***REMOVED******REMOVED*** Inline Code Comments" >> /tmp/github_pr_feedback.md
        echo "" >> /tmp/github_pr_feedback.md
    ...
    ***REMOVED*** Fetch issue comments (general PR discussion)
    local issue_comments
    issue_comments=$(gh api "repos/${GITHUB_REPO}/issues/${pr_number}/comments" --jq '.[] | "***REMOVED******REMOVED******REMOVED*** \(.user.login) (\(.created_at))\n\(.body)\n---"' 2>/dev/null)

    if [ -n "${issue_comments}" ]; then
        echo "***REMOVED******REMOVED*** PR Discussion" >> /tmp/github_pr_feedback.md
        echo "" >> /tmp/github_pr_feedback.md
    ...
    ***REMOVED*** Check if we got any feedback
    if [ -s /tmp/github_pr_feedback.md ]; then
        ...
        if [ "${feedback_lines}" -gt 5 ]; then
            ...
            echo "╠════════════════════════════════════════════════════════════════════╣"
            echo "║ PR ***REMOVED***${pr_number} has feedback that should be addressed.              "
            echo "║ See /tmp/github_pr_feedback.md for full details.                   "
            echo "╚════════════════════════════════════════════════════════════════════╝"
            echo ""
            GITHUB_FEEDBACK_FOUND=true
        else
            echo "[github] No substantive feedback found on PR ***REMOVED***${pr_number}"
...
***REMOVED*** Fetch Jira comments for external feedback
***REMOVED*** Called on startup to get any feedback posted to the Jira ticket
***REMOVED*** Writes feedback to /tmp/jira_comments.md
fetch_jira_comments() {
    ...
    ***REMOVED*** Write comments to file
    echo "***REMOVED*** Jira Comments" > /tmp/jira_comments.md
    echo "" >> /tmp/jira_comments.md
    echo "**Issue: ${JIRA_ISSUE_KEY}**" >> /tmp/jira_comments.md
    echo "" >> /tmp/jira_comments.md

    ***REMOVED*** Extract comments (most recent first)
    echo "${response}" | jq -r '.comments | reverse | .[] | "***REMOVED******REMOVED******REMOVED*** \(.author.displayName) (\(.created))\n\(.body.content[]?.content[]?.text // .body | tostring)\n---"' >> /tmp/jira_comments.md 2>/dev/null
...
case "$WORKER_PROVIDER" in
    ...
    gemini)
        ...
        ***REMOVED*** AI SDK executor uses GEMINI_API_KEY
...
***REMOVED*** =============================================================================
***REMOVED*** Multi-SCM Provider Support
***REMOVED*** =============================================================================
***REMOVED*** Supports GitHub (default), GitLab, and BitBucket
***REMOVED*** Environment variables:
***REMOVED***   SCM_PROVIDER: github | gitlab | bitbucket (default: github)
***REMOVED***   SCM_BASE_URL: For self-hosted instances (e.g., gitlab.company.com)
***REMOVED***   SCM_TOKEN: Access token for the SCM provider
***REMOVED***   BITBUCKET_USERNAME: Required for BitBucket (username:app_password format)
***REMOVED***   GITHUB_TOKEN: Kept for backwards compatibility (used if SCM_TOKEN not set)
...
***REMOVED*** Extract repo info (format: owner/repo or workspace/repo)
REPO_OWNER=$(echo "${GITHUB_REPO}" | cut -d'/' -f1)
REPO_NAME=$(echo "${GITHUB_REPO}" | cut -d'/' -f2)

***REMOVED*** Build clone URL and configure authentication based on provider
case "${SCM_PROVIDER}" in
    ...
    gitlab)
        ...
        ***REMOVED*** Configure glab CLI if available (optional)
    ...
    bitbucket)
        ...
        ***REMOVED*** See: https://support.atlassian.com/bitbucket-cloud/docs/using-access-tokens/
        ***REMOVED*** URL-encode the token (handle = and other special chars)
...
***REMOVED*** =============================================================================
***REMOVED*** Fetch External Feedback (GitHub PR & Jira Comments)
***REMOVED*** =============================================================================
***REMOVED*** Fetch feedback from GitHub PR (reviews, inline comments, discussion) and
***REMOVED*** Jira comments. This feedback will be injected into the prompt for revision
***REMOVED*** runs so the worker can address previous issues.
GITHUB_FEEDBACK_FOUND=false
JIRA_COMMENTS_FOUND=false
EXISTING_PR_NUMBER=""

***REMOVED*** Fetch feedback when:
***REMOVED*** - IS_REVISION_RUN: Previous PR was reviewed and changes were requested
***REMOVED*** - IS_DEPLOYMENT_RUN: PR is approved and we're deploying
***REMOVED*** - RESUMING: Resuming from checkpoint (existing branch)
***REMOVED*** - EXISTING_BRANCH_CHECKED_OUT: Branch already existed remotely (may have a PR with feedback)
...
echo ""
echo "***REMOVED******REMOVED*** 🔍 GitHub PR Feedback"
echo ""
...
echo ""
echo "***REMOVED******REMOVED*** 📝 Jira Comments"
echo ""
...
case "$WORKER_PROVIDER" in
    ...
    ollama|openai|gemini|google|ai-sdk)
        ***REMOVED*** =============================================================================
        ***REMOVED*** Vercel AI SDK Executor
        ***REMOVED*** =============================================================================
        ***REMOVED*** Supports: anthropic, openai, google, gemini, ollama (via ai-sdk-executor.js)
        ***REMOVED*** For ai-sdk mode, AI_SDK_UNDERLYING_PROVIDER specifies the actual provider.
        ***REMOVED*** For direct provider names, WORKER_PROVIDER is passed through.
        ***REMOVED***
...
if [ -f "${OUTPUT_FILE}" ] && [ -s "${OUTPUT_FILE}" ]; then
    ...
    ***REMOVED*** This handles ai-sdk-executor.js output format
    if [ "${INPUT_TOKENS}" = "0" ]; then
