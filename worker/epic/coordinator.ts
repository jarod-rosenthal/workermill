...
export class EpicCoordinator {
    ...
    switch (reviewResult.decision) {
      case "approved":
        ...
        if (this.config.deploymentEnabled) {
          ...
          console.log(`[Epic] ${mergeLabel} — auto-merging PR ***REMOVED***${prNumber} with CI verification`);
  ...
  private async ensureGitHubReviewPosted(
    ...
    try {
      ...
      const body = `***REMOVED******REMOVED*** ${emoji} Tech Lead Review — ${reviewResult.decision.replace("_", " ").toUpperCase()}\n\n` +
      ...
      try {
        ...
        console.log(`[Epic] Posted formal PR review (${event}) to PR ***REMOVED***${prNumber}`);
        ...
        console.log(`[Epic] Posted review as issue comment on PR ***REMOVED***${prNumber}`);
      }
    } catch (err) {
...
Begin your review now. Start by fetching the code changes.`;
  ...
  private async pollPrCI(prNumber: number): Promise<{ passed: boolean; pending: boolean; log?: string }> {
    ...
    try {
      ...
      await this.postLog(`[CI Gate] Polling CI checks on PR ***REMOVED***${prNumber} (SHA: ${headSha.substring(0, 7)})...`);
  ...
  private async mergeWithCIVerification(
    ...
    while (this.ciFixRetryCount < this.maxCiFixRetries) {
      ...
      await this.postLog(
        `[CI Gate] CI failed on PR ***REMOVED***${prNumber} — launching CI Fix Agent (attempt ${this.ciFixRetryCount}/${this.maxCiFixRetries})`
      );
      ...
      if (fixResult.success) {
        ...
        if (ciResult.passed) {
          await this.postLog(`[CI Gate] CI now passing after fix — merging PR ***REMOVED***${prNumber}`);
          const merged = await this.gitOps.mergePR(prUrl, prNumber);
          return { merged };
        }
    ...
    switch (reviewResult.decision) {
      case "approved":
        ...
        if (this.config.prdChildTask) {
          console.log(`[Epic] PRD child task — auto-merging PR ***REMOVED***${prNumber} with CI verification`);
