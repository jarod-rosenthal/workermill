...
export class GitOps {
  ...
  private installPreCommitHook(worktreePath: string): void {
    try {
      ...
      const hookScript = `***REMOVED***!/bin/sh
***REMOVED*** WorkerMill pre-commit hook — enforce .gitignore for build artifacts
***REMOVED*** Ensures build output directories are never committed, even if a worker
***REMOVED*** runs 'git add .' directly via the Bash tool.

GITIGNORE="\$(git rev-parse --show-toplevel)/.gitignore"
ADDITIONS=""

${FORBIDDEN_DIRS.map(d => `***REMOVED*** Ensure ${d} is in .gitignore
...
***REMOVED*** Also unstage .env* files (security)
