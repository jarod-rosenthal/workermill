        ...
        res.on("end", async () => {
          ...
          for (const pr of orphans) {
            try {
              ...
              console.log(`[GitOps] Closed orphan GitHub PR #${pr.number} (${pr.head.ref})`);
              closed++;
            } catch (e) {
              console.warn(`[GitOps] Failed to close GitHub PR #${pr.number}: ${e}`);
            }
          }
        ...
        res.on("end", async () => {
          ...
          for (const pr of orphans) {
            try {
              ...
              console.log(`[GitOps] Declined orphan Bitbucket PR #${pr.id} (${pr.source.branch.name})`);
              closed++;
            } catch (e) {
              console.warn(`[GitOps] Failed to decline Bitbucket PR #${pr.id}: ${e}`);
            }
          }
