...
export class GitOps {
  ...
  /**
   * Post-merge cleanup: close orphan PRs and delete story/feature branches.
  ...
  async postMergeCleanup(jiraKey: string): Promise<void> {
    try {
      ...
      const branchPrefix = `story/${jiraKey.toLowerCase()}/`;
      const scmProvider = this.config.scmProvider || "github";
      const token = this.config.githubToken;
      const [ownerOrWs, repoOrSlug] = this.config.targetRepo.split("/");

      if (!ownerOrWs || !repoOrSlug) {
        console.warn(`[GitOps] Invalid targetRepo for cleanup: ${this.config.targetRepo}`);
        return;
      }

      // 1. Close orphan PRs whose source branch starts with the story prefix
      let closedCount = 0;
      try {
        switch (scmProvider) {
          case "github":
            closedCount = await this.closeGitHubOrphanPRs(ownerOrWs, repoOrSlug, branchPrefix, token);
            break;
          case "bitbucket":
            closedCount = await this.closeBitbucketOrphanPRs(ownerOrWs, repoOrSlug, branchPrefix, token);
            break;
          default:
            console.log(`[GitOps] Orphan PR cleanup not implemented for ${scmProvider}`);
        }
        if (closedCount > 0) {
          console.log(`[GitOps] Closed ${closedCount} orphan PR(s)`);
        }
      } catch (e) {
        console.warn(`[GitOps] Orphan PR cleanup failed: ${e}`);
      }

      // 2. Delete story branches (reuse existing proven method)
      try {
      ...
      // 3. Delete feature branch if still on remote (squash merge doesn't always auto-delete)
      try {
        const featureBranch = `feature/${jiraKey.toLowerCase()}`;
  ...
  private closeGitHubOrphanPRs(
    ...
    return new Promise((resolve) => {
      ...
      const req = https.request(options, (res) => {
        ...
        res.on("end", async () => {
          if (res.statusCode !== 200) {
            console.warn(`[GitOps] GitHub list PRs failed (${res.statusCode}): ${data.substring(0, 200)}`);
            resolve(0);
            return;
          }

          let prs: Array<{ number: number; head: { ref: string } }>;
          try {
            prs = JSON.parse(data);
          } catch {
            console.warn("[GitOps] Failed to parse GitHub PRs response");
            resolve(0);
            return;
          }

          const orphans = prs.filter((pr) => pr.head.ref.startsWith(branchPrefix));
          let closed = 0;

          for (const pr of orphans) {
            try {
              await this.closeGitHubPR(owner, repo, pr.number, token);
              console.log(`[GitOps] Closed orphan GitHub PR ***REMOVED***${pr.number} (${pr.head.ref})`);
              closed++;
            } catch (e) {
              console.warn(`[GitOps] Failed to close GitHub PR ***REMOVED***${pr.number}: ${e}`);
            }
          }
      ...
      req.on("error", (e) => {
        console.warn(`[GitOps] GitHub list PRs request error: ${e.message}`);
        resolve(0);
      });
      req.end();
    });
  ...
  private closeGitHubPR(owner: string, repo: string, prNumber: number, token: string): Promise<void> {
    return new Promise((resolve, reject) => {
      ...
      const req = https.request(options, (res) => {
        ...
        res.on("end", () => {
          if (res.statusCode === 200) {
            ...
            reject(new Error(`GitHub close PR failed (${res.statusCode}): ${data.substring(0, 200)}`));
          }
        });
      });
  ...
  private closeBitbucketOrphanPRs(
    workspace: string,
    repoSlug: string,
    branchPrefix: string,
    token: string,
    ...
    return new Promise((resolve) => {
      ...
      const options: https.RequestOptions = {
        ...
        path: `/2.0/repositories/${workspace}/${repoSlug}/pullrequests?state=OPEN`,
        method: "GET",
        headers: {
          Authorization: authHeader,
      ...
      const req = https.request(options, (res) => {
        ...
        res.on("end", async () => {
          if (res.statusCode !== 200) {
            console.warn(`[GitOps] Bitbucket list PRs failed (${res.statusCode}): ${data.substring(0, 200)}`);
            resolve(0);
            return;
          }

          let response: { values: Array<{ id: number; source: { branch: { name: string } } }> };
          try {
            response = JSON.parse(data);
          } catch {
            console.warn("[GitOps] Failed to parse Bitbucket PRs response");
            resolve(0);
            return;
          }

          const orphans = (response.values || []).filter(
            (pr) => pr.source?.branch?.name?.startsWith(branchPrefix),
          );
          let closed = 0;

          for (const pr of orphans) {
            try {
              // Comment before declining so it's clear this is automated cleanup, not a rejection
              await this.commentBitbucketPR(
                workspace,
                repoSlug,
                pr.id,
                token,
                "Closing: changes were merged via the consolidated PR. This PR is no longer needed.",
              );
              await this.declineBitbucketPR(workspace, repoSlug, pr.id, token);
              console.log(`[GitOps] Declined orphan Bitbucket PR ***REMOVED***${pr.id} (${pr.source.branch.name})`);
              closed++;
            } catch (e) {
              console.warn(`[GitOps] Failed to decline Bitbucket PR ***REMOVED***${pr.id}: ${e}`);
            }
          }
      ...
      req.on("error", (e) => {
        console.warn(`[GitOps] Bitbucket list PRs request error: ${e.message}`);
        resolve(0);
      });
      req.end();
    });
  ...
  private declineBitbucketPR(
    ...
    return new Promise((resolve, reject) => {
      ...
      const options: https.RequestOptions = {
        hostname: "api.bitbucket.org",
        path: `/2.0/repositories/${workspace}/${repoSlug}/pullrequests/${prId}/decline`,
        method: "POST",
        headers: {
          Authorization: authHeader,
          "Content-Length": 0,
      ...
      const req = https.request(options, (res) => {
        ...
        res.on("end", () => {
          if (res.statusCode === 200) {
            ...
            reject(new Error(`Bitbucket decline PR failed (${res.statusCode}): ${data.substring(0, 200)}`));
          }
        });
      });
  ...
  private commentBitbucketPR(
    ...
    return new Promise((resolve) => {
      const body = JSON.stringify({ content: { raw: message } });
      const authHeader = getBitbucketAuthHeader(token);
      const options: https.RequestOptions = {
        hostname: "api.bitbucket.org",
      ...
      const req = https.request(options, (res) => {
        ...
        res.on("end", () => {
          if (res.statusCode !== 201 && res.statusCode !== 200) {
            console.warn(`[GitOps] Bitbucket comment failed (${res.statusCode}): ${data.substring(0, 200)}`);
          }
          resolve();
        });
      });
