...
### Phase 1: Check for Existing CI/CD
...
### Phase 2: Create Workflows if Missing
...
# Get the latest workflow run triggered by the merge
gh run list --branch main --limit 3

# Watch the specific run (blocks until complete)
gh run watch <RUN_ID>

# If watch doesn't work, poll status
...
2. **Check for health endpoint** (if applicable):
   \`\`\`bash
   # If deployment URL is known, curl health endpoint
...
## Instructions - Follow ALL Steps

### Step 1: Check for Existing CI/CD Workflows
...
### Step 2: Create Deployment Workflow (If Missing)
...
jobs:
  deploy:
    ...
    steps:
      ...
      # Add deployment steps based on the project
\`\`\`
...
### Step 4: Monitor Deployment to Completion
...
# Watch the run until it completes (this blocks until done)
gh run watch $RUN_ID
\`\`\`
