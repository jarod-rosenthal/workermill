...
export class InlineEscalationFixer {
  ...
  private buildPrompt(): string {
    ...
    const historyEntries = this.errorHistory
      .map(
        (h) =>
          `### Attempt ${h.attempt} (command: \`${h.failedCommand}\`)\n\`\`\`\n${h.errorSnippet.slice(0, 800)}\n\`\`\``,
      )
      .join("\n\n");
    ...
    if (completions || decisions || fileChanges) {
      coordinationSection = `## Coordination Context — What Other Experts Built
...
`;
      if (completions) {
        coordinationSection += `### Completed Stories\n\n${completions}\n\n`;
      }
      if (fileChanges) {
        coordinationSection += `### Files Created/Modified by Siblings\n\n${fileChanges}\n\n`;
      }
      if (decisions) {
        coordinationSection += `### Technical Decisions\n\n${decisions}\n\n`;
      }
    }
    ...
    return `## Escalation: Repeated Quality Gate Failure
...
## Error History — ${this.errorHistory.length} Failed Attempts
