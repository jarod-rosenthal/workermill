...
export class InlineReviewer {
     ...
     */
    async postLog(message, type = "output") {
     ...
     */
    async review(prUrl, prNumber, revisionCount = 0, previousFeedback) {
     ...
     */
    buildReviewPrompt(prUrl, prNumber, revisionCount, previousFeedback) {
    ...
    handleMessage(msg) {
        if (msg.type === "thinking" && msg.content) {
            ...
            if (msg.toolInput) {
                ...
                if (input.command)
                    toolMsg += ` -> ${String(input.command).substring(0, 100)}`;
                else if (input.file_path)
                    toolMsg += ` -> ${input.file_path}`;
            }
...
//***REMOVED*** sourceMappingURL=inline-reviewer.js.map