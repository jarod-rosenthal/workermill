...
Begin your review now. Start by fetching the PR diff.`;
  ...
  private parseDecision(): ReviewDecision {
    ...
    if (approvalPatterns.some(p => p.test(lowerOutput))) {
      console.log("[tech_lead] Detected natural language approval (missing REVIEW_DECISION marker)");
      return "approved";
    } else if (rejectionPatterns.some(p => p.test(lowerOutput))) {
      console.log("[tech_lead] Detected natural language rejection (missing REVIEW_DECISION marker)");
      return "rejected";
    }
