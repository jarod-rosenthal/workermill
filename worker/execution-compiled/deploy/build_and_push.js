 ...
 * - DOCKER_REGISTRY: ECR registry URL (e.g., 593971626975.dkr.ecr.us-east-1.amazonaws.com/app)
...
if (!tag) {
    try {
        tag = (0, child_process_1.execSync)("git rev-parse --short HEAD", { encoding: "utf-8" }).trim();
    }
    catch {
        tag = `build-${Date.now()}`;
    }
...
if (!registry) {
    ...
    console.error("Example: 593971626975.dkr.ecr.us-east-1.amazonaws.com/myapp");
    process.exit(1);
}
// Resolve dockerfile path
const resolvedDockerfile = path_1.default.resolve(context, dockerfile);
if (!(0, fs_1.existsSync)(resolvedDockerfile)) {
...
try {
    (0, child_process_1.execSync)(`mkdir -p ${dockerConfigDir}`, { stdio: "inherit" });
    (0, child_process_1.execSync)(`echo '${JSON.stringify(dockerConfig)}' > ${dockerConfigPath}`, { stdio: "inherit" });
...
const kanikoArgs = [
    ...
    `--context=${path_1.default.resolve(context)}`,
...
console.log("");
const result = (0, child_process_1.spawnSync)(kanikoArgs[0], kanikoArgs.slice(1), {
    stdio: "inherit",
    env: {
        ...process.env,
        AWS_REGION: region,
