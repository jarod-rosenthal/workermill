...
// This allows workers to deploy to customer's AWS accounts securely
async function setupCredentials() {
    if ((0, cloud_credentials_js_1.hasCustomerAwsConfig)()) {
        console.log("[deploy_ecs] Customer AWS role configured, assuming role...");
        await (0, cloud_credentials_js_1.setCustomerAwsEnvVars)();
        console.log("[deploy_ecs] Now using customer AWS credentials for deployment");
        console.log("");
    }
}
...
async function main() {
    ...
    try {
        ...
        const updateResult = (0, child_process_1.execSync)(`${awsCli} ecs update-service --cluster ${cluster} --service ${service} --force-new-deployment --region ${region}`, { encoding: "utf-8" });
        ...
        if (waitForStable) {
            ...
            console.log("");
            try {
                (0, child_process_1.execSync)(`${awsCli} ecs wait services-stable --cluster ${cluster} --services ${service} --region ${region}`, {
                    encoding: "utf-8",
        ...
        try {
            const describeResult = (0, child_process_1.execSync)(`${awsCli} ecs describe-services --cluster ${cluster} --services ${service} --region ${region}`, { encoding: "utf-8" });
            ...
            if (svc?.events?.length > 0) {
                ...
                svc.events.slice(0, 5).forEach((event) => {
...
// Run main
main().catch((error) => {
    console.error("ERROR: Deployment script failed:", error.message);
    process.exit(1);
});
