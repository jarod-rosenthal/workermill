...
try {
    ...
    try {
        (0, child_process_1.execSync)(`${awsCli} ecs describe-task-definition --task-definition ${targetTaskDef} --region ${region}`, { encoding: "utf-8", stdio: "pipe" });
    ...
    const updateResult = (0, child_process_1.execSync)(`${awsCli} ecs update-service --cluster ${cluster} --service ${service} --task-definition ${targetTaskDef} --region ${region}`, { encoding: "utf-8" });
    ...
    try {
        (0, child_process_1.execSync)(`${awsCli} ecs wait services-stable --cluster ${cluster} --services ${service} --region ${region}`, { encoding: "utf-8", timeout: 600000, stdio: "inherit" });
        console.log("");
