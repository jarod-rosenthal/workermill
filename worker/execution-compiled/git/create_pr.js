...
async function main() {
    ...
    try {
        ...
        // Auto-close linked GitHub Issue when PR merges
        const ticketSystem = process.env.TICKET_SYSTEM || "";
        if (ticketSystem === "github" && ticketKey) {
            const ghIssueNumber = ticketKey.replace(/^(GH-|#)/, "");
            if (/^\d+$/.test(ghIssueNumber)) {
