...
function countLinesOfCode(projectPath) {
    ...
    try {
        const result = exec('find . -name "*.ts" -o -name "*.tsx" -o -name "*.js" -o -name "*.jsx" | xargs wc -l 2>/dev/null | tail -1', projectPath);
...
function runTypecheck(projectPath) {
    const metrics = { errors: 0, passed: true };
    // Check for tsconfig
...
function runTests(projectPath) {
    ...
    if (deps.vitest) {
        ...
        try {
            ...
            if (passMatch)
                metrics.passed = parseInt(passMatch[1]);
            if (failMatch)
                metrics.failed = parseInt(failMatch[1]);
        }
    }
    else if (deps.jest) {
...
function runSecurityAudit(projectPath) {
    ...
    try {
        ...
        if (highMatch)
            metrics.high = parseInt(highMatch[1]);
        if (medMatch)
            metrics.medium = parseInt(medMatch[1]);
        if (lowMatch)
            metrics.low = parseInt(lowMatch[1]);
    }
    return metrics;
}
