...
async function addLinearComment(issueIdentifier, comment) {
    ...
    const issueResponse = await makeRequest("https://api.linear.app/graphql", {
        method: "POST",
        headers: {
    ...
    const commentResponse = await makeRequest("https://api.linear.app/graphql", {
        method: "POST",
        headers: {
            Authorization: linearApiKey,
            "Content-Type": "application/json",
    ...
    try {
        ...
        switch (ticketSystem.toLowerCase()) {
            ...
            case "linear":
                ...
                break;
            case "github":
                // Extract issue number from key (e.g., "***REMOVED***123" or "123")
                const issueNumber = ticketKey.replace(/^***REMOVED***/, "");
