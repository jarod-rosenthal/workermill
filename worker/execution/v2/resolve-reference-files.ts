 ...
 */
function classifyReferenceFile(ref: string): "exact" | "glob" | "description" {
 ...
 */
function buildUnresolvedInstruction(
  ref: string,
  type: "glob" | "description"
): string {
  if (type === "description") {
...
export interface ReferenceFileResolutionResult {
  /** Successfully resolved file paths */
  resolvedPaths: string[];
  /** Instructions for agent to find unresolved files */
...
export async function resolveReferenceFiles(
  ...
  workDir: string
): Promise<ReferenceFileResolutionResult> {
  ...
  for (const ref of referenceFiles) {
    ...
    if (type === "exact") {
      ...
      const fullPath = path.join(workDir, ref);
      try {
