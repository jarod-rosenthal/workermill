...
need_go() {
  ...
  ***REMOVED*** Quality gate commands like "go vet", "go test", "go build" are a clear signal
...
***REMOVED*** Returns target Node.js major version if upgrade needed, empty string otherwise.
***REMOVED*** Checks package.json engines, .nvmrc, .node-version files for a required version
***REMOVED*** higher than the currently installed major version.
detect_node_upgrade() {
  local current_major
  ...
  ***REMOVED*** Check .nvmrc or .node-version in repo root
  if [ -n "$REPO_DIR" ]; then
    ...
    ***REMOVED*** Check root package.json engines.node (e.g. ">=22", "22", "^22")
  ...
  ***REMOVED*** Only upgrade if target is higher than current
...
install_node_version() {
  ...
  ***REMOVED*** Use NodeSource for major version upgrades
...
fallback_scan() {
  ...
  ***REMOVED*** Check CI configs for Node.js version requirements higher than current
...
***REMOVED*** Pass 2b: Node.js version upgrade (if repo declares a higher version than installed)
