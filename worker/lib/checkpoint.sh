...
checkpoint_init() {
    ...
    ***REMOVED*** Use JIRA_ISSUE_KEY for checkpoint keying - this stays constant across retries
    ***REMOVED*** TASK_ID changes on each retry, so checkpoints keyed by TASK_ID are useless for recovery
    local checkpoint_key="${JIRA_ISSUE_KEY:-${TASK_ID:-unknown}}"
    local task_id="${TASK_ID:-unknown}"
    local now=$(date -u '+%Y-%m-%dT%H:%M:%SZ')

    ***REMOVED*** Export checkpoint key for use in other functions
...
checkpoint_load() {
    ...
    ***REMOVED*** Use CHECKPOINT_KEY (Jira issue key) for S3 path - constant across retries
    ...
    if aws s3 cp "${s3_path}" "${CHECKPOINT_FILE}" 2>&1; then
        ***REMOVED*** Validate checkpoint structure
        ...
        if jq empty "${CHECKPOINT_FILE}" 2>/dev/null; then
            ***REMOVED*** Validate checkpoint is valid JSON and increment resume count
            ***REMOVED*** Note: We no longer require exact task ID match since checkpoints are keyed by Jira issue key
            ***REMOVED*** and tasks get new IDs on retry. The checkpoint_key match is implicit via the S3 path.
            if true; then
