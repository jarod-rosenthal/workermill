...
export interface LanguageProfile {
  ...
  /** Output parsers */
  parseTypecheck(stdout: string, stderr: string, exitCode: number): { errors: number; passed: boolean };
  parseLint(stdout: string, stderr: string): { errors: number; warnings: number };
  parseTests(stdout: string, stderr: string): { passed: number; failed: number; skipped: number };
  parseAudit(stdout: string): { high: number; medium: number; low: number };
}
 ...
 */
function detectJsTestRunner(repoPath: string): { test: string; runner: string } {
  const pkgPath = path.join(repoPath, "package.json");
  if (fs.existsSync(pkgPath)) {
    try {
      const pkg = JSON.parse(fs.readFileSync(pkgPath, "utf-8"));
      const deps = { ...pkg.dependencies, ...pkg.devDependencies };
      if (deps?.vitest) return { test: "npx vitest run", runner: "vitest" };
      if (deps?.jest) return { test: "npx jest --forceExit --detectOpenHandles", runner: "jest" };
      if (deps?.mocha) return { test: "npx mocha", runner: "mocha" };
      // If there's a test script, use npm test
      if (pkg.scripts?.test) return { test: "npm test", runner: "npm_test" };
    } catch { /* ignore */ }
  }
...
const typescriptProfile: LanguageProfile = {
  ...
  parseTests(stdout: string, _stderr: string) {
    let passed = 0;
    let failed = 0;
    let skipped = 0;

    // Jest/Vitest: "Tests: X passed, Y failed, Z skipped"
    const testsLineMatch = stdout.match(
    ...
    if (testsLineMatch) {
      ...
      if (passMatch) passed = parseInt(passMatch[1]) || 0;
      if (failMatch) failed = parseInt(failMatch[1]) || 0;
      if (skipMatch) skipped = parseInt(skipMatch[1]) || 0;
    }
...
const rustProfile: LanguageProfile = {
  ...
  parseTypecheck(_stdout: string, stderr: string, exitCode: number) {
...
const javaProfile: LanguageProfile = {
  id: "java",
  displayName: "Java",
  ...
  parseTests(stdout: string, _stderr: string) {
    ...
    const surefireMatch = stdout.match(
...
const rubyProfile: LanguageProfile = {
  id: "ruby",
  displayName: "Ruby",
  markers: ["Gemfile"],
  extensions: [".rb"],
...
export const ALL_PROFILES: Record<string, LanguageProfile> = {
 ...
 */
export function detectLanguage(repoPath: string): LanguageProfile {
  ...
  const exists = (file: string) => fs.existsSync(path.join(repoPath, file));

  // Detection order: most specific first
  if (exists("Cargo.toml")) return rustProfile;
  if (exists("go.mod")) return goProfile;
  if (exists("pyproject.toml") || exists("requirements.txt") || exists("setup.py")) return pythonProfile;
  if (exists("Gemfile")) return rubyProfile;
  if (exists("pom.xml") || exists("build.gradle") || exists("build.gradle.kts")) return javaProfile;
 ...
 */
export function detectLanguageWithTestRunner(repoPath: string): LanguageProfile {
  const profile = detectLanguage(repoPath);

  // Only customize test runner for TypeScript
  if (profile.id !== "typescript") return profile;
  ...
  return {
    ...
    testTargeted:
      runner === "vitest"
        ? (files: string[]) => `npx vitest run --related ${files.join(" ")} 2>&1`
        : runner === "jest"
          ? (files: string[]) => `npx jest --findRelatedTests ${files.join(" ")} --ci 2>&1`
          : null,
  };
}
 ...
 */
export function findGoModDirs(repoPath: string): string[] {
  const dirs: string[] = [];
  if (fs.existsSync(path.join(repoPath, "go.mod"))) dirs.push(repoPath);
  try {
