...
#
# Routing:
# - Anthropic provider: Uses Agent SDK (Node.js) for native Claude Code integration
# - Other providers: Uses shell-based approach with AI SDK executor
...
# Route to Agent SDK for Anthropic (default)
if [ "${MANAGER_PROVIDER}" = "anthropic" ] || [ -z "${MANAGER_PROVIDER}" ]; then
    echo "[Manager] Using Agent SDK mode (Node.js)"

    # Run the Agent SDK-based manager
    exec node /app/manager/dist/index.js
fi

# Fall through to shell-based approach for non-Anthropic providers
...
EOF
    ...
    "analyze_logs")
        ...
        cat >> "${INSTRUCTIONS_FILE}" << EOF
# Current Task: Log Analysis (Manager Mode)
...
if [ -n "${CLAUDE_MODEL}" ]; then
    ...
    # Default model per action
    case "${MANAGER_ACTION}" in
        "review_pr")
            CLAUDE_MODEL="opus"  # Best reasoning for code review
            ;;
        "analyze_logs")
            CLAUDE_MODEL="haiku"  # Fast for pattern extraction
            ;;
    esac
fi
...
# Run AI Agent (Claude CLI or AI SDK based on provider)
CLAUDE_OUTPUT_FILE="/tmp/claude-output.jsonl"

# Route based on provider setting
if [ "${MANAGER_PROVIDER}" = "anthropic" ] || [ -z "${MANAGER_PROVIDER}" ]; then
    # Agent SDK (Claude Code CLI) - proven working for Anthropic
    ...
    node /app/agents/ai-sdk-executor.js \
        ...
        2>/tmp/claude-stderr.log | tee "${CLAUDE_OUTPUT_FILE}" | while IFS= read -r line; do
        # Log all output from AI SDK executor
        if [ -n "$line" ]; then
            echo "$line"
