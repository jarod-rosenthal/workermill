***REMOVED***!/bin/bash
***REMOVED*** Ralph Execution Helper Script
***REMOVED*** Orchestrates the Ralph PRD-to-code workflow
***REMOVED*** Called from entrypoint.sh when USE_RALPH=true

set -e

***REMOVED*** =============================================================================
***REMOVED*** Log Streaming Functions
***REMOVED*** =============================================================================

***REMOVED*** Post log helper (passed from parent script)
post_log() {
...
***REMOVED*** Post Ralph-specific log with [ralph] prefix
post_ralph_log() {
    local message="$1"
...
***REMOVED*** Last known progress state (to avoid duplicate logging)
LAST_PROGRESS_MARKER=""
LAST_COMPLETED_COUNT=0

***REMOVED*** Parse and report progress from .ralph/progress.json
check_progress() {
    ...
    ***REMOVED*** Run progress parser if available
    if [ -f "/app/ralph/parse-progress.sh" ]; then
        ...
        if [ -n "$PROGRESS_OUTPUT" ]; then
            ***REMOVED*** Extract progress marker
            PROGRESS_MARKER=$(echo "$PROGRESS_OUTPUT" | grep "::ralph_progress::" | head -1 || true)

            ***REMOVED*** Only log if progress changed
            if [ -n "$PROGRESS_MARKER" ] && [ "$PROGRESS_MARKER" != "$LAST_PROGRESS_MARKER" ]; then
                ***REMOVED*** Parse the marker: ::ralph_progress::<current>/<total>::<description>
            ...
            ***REMOVED*** Check for completed stories count
...
***REMOVED*** =============================================================================
***REMOVED*** Activity Log Streaming
***REMOVED*** =============================================================================

***REMOVED*** Start streaming Ralph activity log with robust file handling
***REMOVED*** Handles: file not existing initially, truncation, rotation
start_activity_log_streaming() {
    local log_file="/workspace/repo/.ralph/activity.log"

    (
        ***REMOVED*** Wait for log file to be created (with timeout)
        WAIT_COUNT=0
        ...
        ***REMOVED*** Use tail -F (capital F) to follow file even through truncation/rotation
        ***REMOVED*** --retry keeps trying if file becomes inaccessible temporarily
        tail -F --retry "$log_file" 2>/dev/null | while IFS= read -r line; do
            ***REMOVED*** Skip empty lines
            [ -z "$line" ] && continue

            ***REMOVED*** Prefix all Ralph activity logs
            post_ralph_log "$line"
        done
    ) &
...
***REMOVED*** Start progress monitoring loop
...
***REMOVED*** =============================================================================
***REMOVED*** Claude Code Log Streaming (from Ralph subprocess)
***REMOVED*** =============================================================================

***REMOVED*** Stream Claude Code output from Ralph's subprocess
***REMOVED*** Ralph invokes Claude Code which writes to its own log/output
***REMOVED*** We capture this via Ralph's activity log which includes Claude output
stream_ralph_output() {
    local output_file="$1"

    if [ -f "$output_file" ]; then
        ***REMOVED*** Stream output through to console (and thus to entrypoint's log parser)
...
***REMOVED*** Ensure .ralph directory exists
mkdir -p /workspace/repo/.ralph

***REMOVED*** Step 1: Convert Jira ticket to PRD format
post_log "system" "Step 1: Converting Jira ticket to PRD format..." "info"

***REMOVED*** Build Jira ticket JSON for the converter
...
***REMOVED*** Compile and run the Jira-to-PRD converter
...
***REMOVED*** Step 2: Create Ralph configuration
post_log "system" "Step 2: Setting up Ralph configuration..." "info"

***REMOVED*** Copy and customize Ralph config
if [ -f "/app/ralph/config.template.json" ]; then
    ***REMOVED*** Substitute environment variables in config template
...
***REMOVED*** Step 3: Run Ralph planning
...
if command -v ralph >/dev/null 2>&1; then
    ...
    ralph plan --config .ralph/config.json 2>&1 | while IFS= read -r line; do
        ***REMOVED*** Stream planning output with [ralph] prefix
    ...
    ***REMOVED*** Check planning results
...
***REMOVED*** Step 4: Run Ralph execution loop
post_log "system" "Step 4: Starting Ralph execution loop..." "info"
post_ralph_log "Starting execution loop..."

***REMOVED*** Start background activity log streaming (with rotation handling)
...
***REMOVED*** Run Ralph loop - stream output directly
***REMOVED*** Ralph's loop command invokes Claude Code for each story
***REMOVED*** The activity.log captures Ralph-level events, while Claude's output
***REMOVED*** goes through Ralph's configured agent command
RALPH_LOOP_EXIT=0
ralph loop --config .ralph/config.json 2>&1 | while IFS= read -r line; do
    ***REMOVED*** Skip empty lines
    [ -z "$line" ] && continue

    ***REMOVED*** Detect story transitions from Ralph output
    if echo "$line" | grep -qi "starting story\|beginning story"; then
        post_ralph_log "$line"
    elif echo "$line" | grep -qi "completed story\|finished story"; then
        post_ralph_log "$line"
        ***REMOVED*** Check progress after story completion
        check_progress
    elif echo "$line" | grep -qi "error\|failed\|exception"; then
        post_ralph_log "$line" "warning"
    else
        ***REMOVED*** Regular output - could be Claude Code output
        ***REMOVED*** Pass through without [ralph] prefix if it looks like Claude output
...
***REMOVED*** Check Ralph loop exit status
...
***REMOVED*** Final progress check
check_progress

***REMOVED*** Parse final results from progress.json
FINAL_RESULT="deployed"
if [ -f "/workspace/repo/.ralph/progress.json" ]; then
    ***REMOVED*** Get final status
    ...
    ***REMOVED*** Map Ralph status to WorkerMill result
    case "$RALPH_STATUS" in
        ...
        failed|error)
            ...
            ;;
        *)
            ***REMOVED*** Check if we completed all stories
    ...
    ***REMOVED*** Extract PR URL if created by Ralph
...
***REMOVED*** Check activity log for errors (fallback detection)
