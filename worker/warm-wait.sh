***REMOVED***!/bin/bash
***REMOVED*** Warm Container Wait Script
***REMOVED*** Polls the API for task assignment, then sources environment variables and re-runs start.sh
...
***REMOVED*** Register as ready with the API
...
***REMOVED*** Check if registration was successful
...
***REMOVED*** Poll for assignment (30s long-poll with 35s timeout)
POLL_COUNT=0
MAX_POLLS=720  ***REMOVED*** 6 hours max wait time (720 * 30s = 6 hours)

while [ ${POLL_COUNT} -lt ${MAX_POLLS} ]; do
    POLL_COUNT=$((POLL_COUNT + 1))

    ***REMOVED*** Long-poll the API (30s server-side timeout, 35s client timeout)
    ...
    case "${STATUS}" in
        "assigned")
            ...
            ***REMOVED*** Extract environment variables and write to a file
            ***REMOVED*** Use jq's @sh filter for proper shell escaping to prevent injection
            echo "${RESPONSE}" | jq -r '.environment // {} | to_entries[] | "export " + .key + "=" + (.value | @sh)' > /tmp/task_env.sh

            ***REMOVED*** Source the environment variables
            set -a
            source /tmp/task_env.sh
            set +a

            ***REMOVED*** Clean up the temp file
            rm -f /tmp/task_env.sh

            ***REMOVED*** Clear warm mode flag so start.sh doesn't loop back here
            ...
            ***REMOVED*** Re-run start.sh which will now route to the appropriate executor
            exec /app/start.sh
            ;;
        ...
        "waiting")
            ***REMOVED*** Still waiting, continue polling
            ***REMOVED*** Send heartbeat every poll (built into the poll endpoint)
            ;;

        "error")
            MESSAGE=$(echo "${RESPONSE}" | jq -r '.message // "unknown error"')
            echo "[Warm Pool] Poll error: ${MESSAGE}"
            ***REMOVED*** Continue polling unless it's a fatal error
            sleep 5
            ;;
